...
/*! RESOURCE: PnmacCheckoutValidation */
function PnmacCheckoutValidation() {}
PnmacCheckoutValidation.prototype = {
  validateCheckout: function(context, form) {
    showLoadingDialog();
    var ga = new GlideAjax('PnmacCheckoutValidationAjax');
    ga.addParam('sysparm_name', 'validate');
    ga.addParam('sysparm_requested_for', $('sc_cart.requested_for').value + '');
    ga.addParam('sysparm_cart', $('sc_cart.sys_id').value + '');
    var parent = this;
    ga.getXML(function(response) {
      ...
      if (answer.result == 'warning') {
        hideLoadingDialog();
        parent._showWarningDialog("Warning", answer);
        return false;
      } else if (answer.result == 'error') {
        hideLoadingDialog();
        parent._showErrorDialog("Error", answer);
        return false;
      } else if (answer.result == 'fatal') {
        hideLoadingDialog();
        parent._showErrorDialog("System Error", answer);
        return false;
      }
      hideLoadingDialog();
  ...
  _showWarningDialog: function(title, answerData) {
    var modal = this._createDialog(null, 500);
    modal.setTitle(title);
    var body = document.createElement('div'),
      header = document.createElement('p'),
      footer = document.createElement('div');
    header.innerHTML = answerData.errorMessage;
    body.appendChild(header);
    if (answerData.items) {
      var listGroup = document.createElement('div');
      listGroup.className = "list-group";
      for (var i = 0; i < answerData.items.length; i++) {
        var listGroupItem = document.createElement('a'),
          listGroupItemHeader = document.createElement('div'),
          listGroupItemText = document.createElement('div');
        listGroupItem.className = "list-group-item";
        listGroupItem.target = "_blank";
        listGroupItem.href = answerData.items[i].sc_req_item.url;
        listGroupItemHeader.className = "list-group-item-header";
        listGroupItemText.className = "list-group-item-text";
        listGroupItemHeader.innerHTML = answerData.items[i].sc_req_item.number + " " +
          answerData.items[i].sc_req_item.cat_item;
        listGroupItemText.innerHTML = answerData.items[i].sc_req_item.summary + "<br>" +
          "Submitted by " + answerData.items[i].sc_req_item.opened_by + " on " + answerData.items[i].sc_req_item.opened_at;
        listGroupItem.appendChild(listGroupItemHeader);
    ...
    var footerMessage = answerData.footerMessage,
      footerMessageBlock = document.createElement("span");
    footerMessageBlock.innerHTML = footerMessage;
    footerMessageBlock.style.paddingRight = "10px";
    var cancelButton = document.createElement('button');
    cancelButton.innerHTML = "Cancel";
    cancelButton.className = "btn btn-default";
    cancelButton.setAttribute("data-dismiss", "GlideModal");
    var okButton = document.createElement('button');
    okButton.innerHTML = "Checkout";
    okButton.className = "btn btn-primary";
    okButton.onclick = function() {
      var form = document.getElementById('sc_cart.do');
      checkout(this, form);
    };
    footer.appendChild(footerMessageBlock);
    footer.appendChild(cancelButton);
    footer.appendChild(okButton);
    body.appendChild(footer);
    modal.setBody(body);
  },
  _showErrorDialog: function(title, answerData) {
    var modal = this._createDialog(null, 500);
    modal.setTitle(title);
    var body = document.createElement('div'),
      header = document.createElement('p'),
    ...
    if (answerData.items) {
      ...
      for (var i = 0; i < answerData.items.length; i++) {
        ...
        listGroupItemHeader.className = "list-group-item-header";
        listGroupItemText.className = "list-group-item-text";
        listGroupItemHeader.innerHTML = answerData.items[i].sc_cart_item.cat_item;
        listGroupItemText.innerHTML = answerData.items[i].sc_cart_item.summary;
        if (answerData.items[i].conflicts)
          listGroupItemText.innerHTML += '<br>Error Reason: ' + answerData.items[i].conflicts;
        listGroupItem.appendChild(listGroupItemHeader);
        listGroupItem.appendChild(listGroupItemText);
        listGroup.appendChild(listGroupItem);
      }
  ...
  _createDialog: function(name, width) {
    name = (!name) ? "custom_dialog" : name;
    width = (!width) ? 300 : width;
    var dialogClass = window.GlideModal ? GlideModal : GlideDialogWindow;
    return new dialogClass(name, false, width);
  },
  validateRequestedFor: function(context) {
    this._clearFieldMessages();
    if (!context.value) {
      this._disableCheckout();
      var displayId = "sys_display." + context.id,
        displayInput = document.getElementById(displayId);
      if (displayInput.value) {
        this._showFieldMessage("sc_cart.requested_for", "Please enter a valid employee name", "error");
      } else {
        this._showFieldMessage("sc_cart.requested_for", "Please enter an employee name", "error");
      }
    } else {
      if (context.value.length === 32)
        this._enableCheckout();
      else {
        this._disableCheckout();
        this._showFieldMessage("sc_cart.requested_for", "An unexpected error occurred", "error");
      }
    }
  },
  _clearFieldMessages: function() {
    $$('.fieldmsg').each(function(el) {
      el.remove();
    });
  },
  _disableCheckout: function() {
    $('header_button_checkout_in_header').disabled = true;
    $('header_button_checkout_in_footer').disabled = true;
  },
  _enableCheckout: function() {
    $('header_button_checkout_in_header').disabled = false;
    $('header_button_checkout_in_footer').disabled = false;
  },
  _showFieldMessage: function(input, message, type, scrollForm) {
    var msgClass;
    var msgImage;
    var msgImageAlt;
    var msgRowType;
    switch (type) {
      case "info":
        msgClass = g_form.INFO_CLASS;
        msgRowType = 'info' + g_form.MSG_ROW;
        break;
      case "error":
        msgClass = g_form.ERROR_CLASS;
        msgRowType = 'error' + g_form.MSG_ROW;
        break;
      case "warning":
        msgClass = g_form.WARNING_CLASS;
        msgRowType = 'warning' + g_form.MSG_ROW;
        break;
      default:
        msgClass = g_form.INFO_CLASS;
        msgRowType = 'info' + g_form.MSG_ROW;
    ...
    var positionedCursor = g_form._positionCursorAtError(inputElement, message);
    var fieldRow = inputElement.up('div.sc_cv_info_row');
    var doc;
    ...
    if (!positionedCursor)
      g_form._scrollToElementTR(fieldRow, fieldMsgRow, scrollForm);
    _frameChanged();
  },
  type: "PnmacCheckoutValidation"
};
...
/*! RESOURCE: PnmacVariableEditUtils */
function PnmacVariableEditUtils() {}
PnmacVariableEditUtils.prototype = {
  toggleRequested: function() {
    var ve = $('variable_map').up('table');
    this._toggleElements(ve);
    this._toggleReferenceIcons(ve);
    this._toggleCollectorFilter(ve);
  },
  toggleItemInViewport: function() {
    var ve = $('catalog_item_viewport').contentWindow.$('variable_map').up('table');
    this._toggleElements(ve);
    this._toggleReferenceIcons(ve);
    this._toggleCollectorFilter(ve);
  },
  ...
  _toggleElements: function(ve) {
    ve.select('.cat_item_option',
        '.questionsetreference',
        '.slushbucket select',
        '.slushbucket input',
        '.m2m_filter_container button',
        '.slushbucket .icon-chevron-right',
        '.slushbucket .icon-chevron-left',
        'input.checkbox')
      .each(function(elmt) {
        if (elmt.hasAttribute("disabled")) {
          elmt.removeAttribute("disabled");
        } else {
          elmt.setAttribute("disabled", "");
        }
      });
  },
  _toggleReferenceIcons: function(ve) {
    ve.select('.icon-search', '.icon-calendar').each(function(img) {
  ...
  _toggleCollectorFilter: function(ve) {
    ve.select('.m2m_filter_container').each(function(e) {
...
/*! RESOURCE: PnmacSlushbucketModificaitons */
...
function hideSBFilter() {
  console.log("Hiding slushbucket filter...");
  var ve = $('variable_map').up('table');
  ve.select('.m2m_filter_container').each(function(e) {
    e.style.display = 'none';
  });
...
function modifyLabels() {
  console.log("Modifying slushbucket labels...");
  var ve = $('variable_map').up('table');
  ve.select('.slushbucket label').each(function(e) {
    var forVal = e.getAttribute('for') + '';
    if (forVal.indexOf('_select_0') > -1)
      e.innerHTML = 'Available';
    else if (forVal.indexOf('_select_1') > -1)
      e.innerHTML = 'Selected';
  });
}

function setSBFilter(name, filter) {
  var collectorName = name;
...
function clearSBSelection(varName) {
...
function setSBSelection(varName, value) {
  clearSBSelection(varName);
  var leftBucket = gel(varName + '_select_0');
  var rightBucket = gel(varName + '_select_1');
  var selectedOptions = leftBucket.options;
  rightBucket.removeChild(rightBucket.options[0]);
  var selectedIDs = [],
    selectedValues = value.split(',');
  var index = 0;
  for (var i = 0; i < selectedOptions.length; i++) {
    for (var j = 0; j < selectedValues.length; j++) {
      if (selectedOptions[i].value === selectedValues[j]) {
        selectedIDs.push(i);
        break;
      }
    }
...
function delaySetSBSelection(varName, value) {
  var leftBucket = gel(varName + '_select_0');
  if (leftBucket.options[0] && leftBucket.options[0].innerHTML != "Loading")
    setSBSelection(varName, value);
  else {
    setTimeout(function() {
      delaySetSBSelection(varName, value);
    }, 500);
  }
}

function setVariableValuesFromData(data) {
  if (data) {
    try {
      data = JSON.parse(data);
    } catch (e) {
      g_form.addErrorMessage("Error parsing data.");
      return;
    }
    for (var v in data) {
      var collector = document.getElementById(v + "_select_0");
      if (collector) {
        if (collector.options[0] && collector.options[0].innerHTML != "Loading")
          setSBSelection(v + '', data[v] + '');
        else
          delaySetSBSelection(v + '', data[v] + '');
      } else
        g_form.setValue(v, data[v]);
    }
  }
}
...
/*! RESOURCE: pnmacQuickListQuery */
function pnmacQuickListQuery(table, query, field, formField, callback) {
  var ajax = new GlideAjax('GlideRecordQueryAjax');
  ajax.addParam('sysparm_name', 'grQuery');
  ajax.addParam('sysparm_table', table);
  ajax.addParam('sysparm_query', query);
  ajax.addParam('sysparm_field', field);
  ajax.addParam('sysparm_form_field', formField);
  ajax.getXML(function(response) {
    grQueryResponse(response, callback);
  });
}

function grQueryResponse(response, callback) {
  var result = response.responseXML.getElementsByTagName("result")[0],
    meta = response.responseXML.getElementsByTagName("metadata")[0];
  if (result && meta) {
    var formField = meta.getAttribute('formfield'),
      value = result.getAttribute('value'),
      dValue = result.getAttribute('dv');
    callback(formField, value.split(','), dValue.split(','));
  } else if (meta) {
    var formField = meta.getAttribute('formfield');
    callback(formField, null, null);
  }
}
