...
AssessmentUtilsAJAX.prototype = Object.extendsObject(AbstractAjaxProcessor, {
	...
	getFilterTables : function() {
		...
		for (var i = 0; i < labelsAndValues.length; i++) {
			label.push(labelsAndValues[i].label);
			value.push(labelsAndValues[i].value);
		}
		var result = this.newItem("result");
	...
	getTableFields : function() {
		...
		for (var i = 0; i < combos.length; i++) {
			if (!combos[i].value || fieldsProcessed[combos[i].value])
				continue;
			var result = this.newItem('result');
			result.setAttribute('value', combos[i].value);
			result.setAttribute('label', combos[i].label);
			result.setAttribute('display', combos[i].display);
			result.setAttribute('referenceKey', combos[i].referenceKey);
			result.setAttribute('reference', combos[i].reference);
			fieldsProcessed[combos[i].value] = true;
		}
	},
