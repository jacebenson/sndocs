...
UserSubscriptionHelper.prototype = {
	...
	addUserToLicenseWithStatus: function(userSysID, licenseSysID, userSetSysID, status) {
		...
		var isUserAddedToPending = false;
		...
		if (JSUtil.notNil(status) && status === this.LICENSE_STATUS_APPROVAL_REQUIRED) 
			isUserAddedToPending = this.addUserToPendingLicense(userSysID, licenseSysID, false);
		else 
			isUserSubscribed = this._allocateUserToLicense(userSysID, licenseSysID, userSetSysID);
		
		isUserAdded = isUserAddedToPending || isUserSubscribed;
