...
PwdVerifyIdentity.prototype = {
	...
	verifyIdentity: function(sysparm_process_id, sysparm_user_id, sysparm_captcha) {
		...
		// Return error if the process emails a password reset url, but the user has disabled notifications, or url notification for primary email
		if (process.getEmailPasswordResetUrl()) {
		...
		/* ---  This section is no longer required, as it is covered by the following section ---
		// Return error if the user is not sufficiently enrolled for this process:
		var isEnrolled = enrollMgr.isUserEnrolledByProcessId(userSysId, sysparm_process_id, request_id);
		if (!isEnrolled) {
		...
		*/
		...
		// Placing the enrolled verifications into a session variable, for future use.
		gs.getSession().putProperty('sysparm_user_verifications', userVerifications);
