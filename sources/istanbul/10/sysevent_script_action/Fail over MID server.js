...
function failOver() {
	...
	try {
					...
					case GlideappIECC.ECC_AGENT_SELECTOR_MODE_SPECIFIC_CLUSTER+'':
						...
						try {
							...
							if (String(e).indexOf(NO_MID_FOUND_EXCEPTION) != -1) {
								...
								// Send cluster down event
								gs.eventQueue('mid_server.cluster.down', clusterGr, clusterGr.name+'', currMidName);
							} else throw e;
						}
						break;
					default:
		...
		for (var name in newMidNames)
			new MIDServerManage().resetQueryWindow(name);
