	...
	function setInputHandlers(input, paramObj) {
		input.blur(paramObj, function(evt) {
			var blurExec = function() {
				...
				var removeAll = (lastEvent === false && evt.data.type === 'create');
				processLastEvent();
			...
			} else if (!evt.relatedTarget || evt.relatedTarget.id === 'ok_button' || $j(evt.relatedTarget).is('.list-item')) {
				/*
	...
	function updateEvtHandler(event) {
		...
		if (newValue && newValue.length && newValue !== oldValue) {
			...
			if (event.data.type === 'update') {
				...
				var dataItem = findItemById(settings.jsonData, sysId);
