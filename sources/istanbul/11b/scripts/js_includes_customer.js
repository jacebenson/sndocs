...
addLoadEvent(function() {
  ...
  function scSplitView(num_of_cols) {
    ...
    $j.each(tableRowElements, function(index, value) {
      if (value.textContent.trim() === "Categories" || value.textContent.trim() === "Categorías" || value.textContent.trim() === "Categorieën" || value.textContent.trim() === "Catégories" || value.textContent.trim() === "Kategorien") {
      ...
      if (value.textContent.trim() === "Items" || value.textContent.trim() === "Elementos" || value.textContent.trim() === "Éléments" || value.textContent.trim() === "Elemente") {
        tableRowElements.eq(index).after(itemMainRow);
        itemRows.append($j("<td width='50%' style='vertical-align: top;font-size:90%'>").append($j("<table class='wide'  style='margin-top: 25px;'>")));
        itemRows.append($j("<td width='50%' style='vertical-align: top;font-size:90%'>").append($j("<table class='wide'  style='margin-top: 25px;'>")));
        $j(".no_underline_table").remove();
        $j(".sc_category_item").css({
          "border-bottom": "none",
          "padding-top": "10px"
        });
        itemsIndex = index;
      ...
      if (items && (itemsIndex !== index)) {
        var indexCols = index % num_of_cols;
        if (index % num_of_cols) {
      ...
      if (index === (tableRowElements.length - 1)) {
        itemRows.find("table.wide").eq(0).append(value);
        return false;
      }
    });
...
/*! RESOURCE: (ASP) CMS Hide "Back to Catalog" button */
if (location.href.indexOf("/bssc/") != -1 &&
  (location.href.indexOf("/servicecatalog_checkout_one.do") != -1 || location.href.indexOf("/com.glideapp.servicecatalog_cart_view.do") != -1)) {
  addLoadEvent(aspHideBackToCatalogButtonInCMS);
}

function aspHideBackToCatalogButtonInCMS() {
  $$(".request_catalog_button").each(
    function(element) {
      if (element.onclick && String(element.onclick).indexOf("catalog_home.do") != -1) {
        element.hide();
      }
    });
}
/*! RESOURCE: AspLanguageSelect */
var AspLanguageSelect = Class.create();
AspLanguageSelect.prototype = {
  ...
  _add: function(items) {
    ...
    try {
      if (GlideManager.get().hasProperty('glide.ui.nav.stripe.select.maxchars'))
        maxChars = parseInt(GlideManager.get().getProperty('glide.ui.nav.stripe.select.maxchars'), 10);
    } catch (e) {}
    var found = false;
    ...
    if (!found) {
      var label = maxChars && currentLang.length > maxChars ? currentLang.substring(0, maxChars) + ' ...' : currentLang;
...
/*! RESOURCE: PopupClose */
function popupClose() {
  if (popupCurrent == null || (popupCurrent.location && popupCurrent.location.href.indexOf("email_client.do") != -1)) {
    return;
  }
  try {
...
/*! RESOURCE: DisableEnableChoiceOptions */
