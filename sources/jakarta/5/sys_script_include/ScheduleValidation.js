...
ScheduleValidation.warnEmptySchedules = function(cmn_schedule) {
   var msg = gs.getMessage('Warning: There are no active entries in this Schedule');
   var sv = new ScheduleValidation(cmn_schedule);
   if (!sv.isEmptySchedule())
      return;

   if (sv.hasChildSchedule())
         msg = gs.getMessage('Warning: There are no active entries in this Schedule, or its Child Schedules');
...
ScheduleValidation.prototype = {
   ...
   isEmptySchedule: function() {
      var activeSpans = new ScheduleValidation(this.cmn_schedule.sys_id).countActiveSpans();
      ...
      while (gr.next() && activeSpans === 0) {
         this._hasChild = true;
         // only 'include' types should be checked
         // but we are interested to know whether there are any child schedules at all
         if (gr.type == 'include')
            activeSpans += new ScheduleValidation(gr.child_schedule).countActiveSpans();
      }
