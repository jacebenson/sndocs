...
SLAProperties.MODULES_AND_CATEGORIES_2010 = {
	...
	"propertyCategories": {"Service Level Management" : {
		                       "title" : gs.getMessage('{0}You are using the 2010 SLA engine and the configurable properties for this engine are displayed below{1}', ['<p><em><font size="4">', '</font></em></p><p><em><font size="3">']) + 
										 gs.getMessage('It is recommended that you move to the 2011 SLA Engine.  Click {0}Move from 2010 Engine to the 2011 engine{1}here{2} to view the instructions to move.{3}', ['<a title="','" href="' + SLAProperties.MOVING_TO_2011_HELP + '" target="_blank" rel="nofollow">','</a>','</em></p><hr/>']),
	...
	for (var categoryName in propertyCategories) {
		...
		if (propertyCategoryGr.get("name", categoryName)) {
			...
			if (propertyCategory.hasOwnProperty("sys_properties")) {
				...
				var properties = propertyCategory.sys_properties;
...
SLAProperties.setBreachCompat = function(value) {
	...
	gs.setProperty('com.snc.sla.compatibility.breach', value, gs.getMessage('{0}Enable compatibility with 2010 \'breached\' stage for SLAs{1}Set to yes to set a Task SLA\'s stage field to {2}breached{3} when it exceeds the breach time{4}', ['<span style="font-weight:bold;font-size: larger">', '<br/></span>','<b>','</b>','<br/>']) +
															  gs.getMessage('{0}Note: {1}this is legacy behaviour as the 2011 engine sets the {0}Has breached{1} field to indicate that a Task SLA has exceeded the breach time', ['<b>','</b>']));
};
