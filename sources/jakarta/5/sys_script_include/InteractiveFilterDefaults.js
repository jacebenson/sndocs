...
InteractiveFilterDefaults.prototype = Object.extendsObject(AbstractAjaxProcessor, {
	...
	getQueryPart: function(condition){
	  ...
	  var query = condition.filter;

	  var qs = new GlideQueryString(table,query);
	      qs.deserialize();
	      qs.removeGroupBy();
	  ...
	  for (var i =0;i<terms.size();i++){
	     var field = terms.get(i).getTermField();
		 var vals = terms.get(i).getValue(); 
		 if(typeof queryPart[field] !== "undefined") {
			 queryPart[field] = vals.split(",").concat(queryPart[field]);
		 }
		 else {
			 queryPart[field] = vals.split(",");
		 }
	  }
