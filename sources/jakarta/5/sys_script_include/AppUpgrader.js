...
AppUpgrader.prototype = {
	...
	_checkCompatibility: /*void*/ function(/*sys_store_app*/ storeApp, /*String*/ version) {
		...
		for (var i = 0; i < depArray.length; i++) {
			...
			if (depPair.length < 2 || depPair[1] == "sys") { // handle plugin dependencies
				...
				if (this._scopeAlreadyActive(depPair))
					continue;
	...
	_scopeAlreadyActive: /*boolean*/ function(/*[]*/ depPair) {
		var scope = depPair[0];
		var wantVersion = depPair[1];
		var haveVersion;
		
		var availableScopes = new GlideRecord("sys_scope");
		availableScopes.addQuery("scope", scope);
		availableScopes.query();
		
		if (!availableScopes.next())
			return false;
		
		haveVersion = availableScopes.getValue("version");
