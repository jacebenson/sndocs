...
TaskSLAController.prototype = {
      ...
      var taskSLAgr = taskSLA.getGlideRecord();
        ...
        if (completeMatches || resetMatches) {
			...
			if (resetMatches) {
				...
				if (!skipResetAction) {
					var newTaskSLA = SelfCleaningMutex.enterCriticalSectionRecordInStats(this.MUTEX_NEW + this.taskGR.sys_id, this.MUTEX_NEW,
					this, this._checkNewSLA, taskSLAgr.sla.getRefRecord());
  ...
  _adjustPauseTime: function(taskSLAgr) {
      ...
      taskGR.autoSysFields(false);
        var historyGR = this._rebuildHistory(this.taskGR);
    ...
    _rebuildHistory: function(taskGR, updateTime) {
	  ...
	  if (updateTime)
		  historyGR.addQuery('update_time', '<=', updateTime);
