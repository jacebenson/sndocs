...
TourBuilderObjectDataFactory.prototype = {
	...
	*/
	getTourdetails : function(args){
		...
		if(gtbRecordingDisabled == "false"){
			...
			tourSysId = this._df.getObjects({'table' :'tour_builder_guide', 'query_params' : query_params});
		} else{
			query_params.push({"column" : "sys_id", "value" : args.tour_id });
	...
	*/
	getToursteps : function(args){	
		...
		if(gtbRecordingDisabled == "false") {
			query_params.push({"column" : "tour_id", "value" : args.tour_id });
			stepSysIds = this._df.getObjects({'table' :'tour_builder_step', 'query_params' : query_params});
		} else {
