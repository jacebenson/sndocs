...
MIDServerCluster.prototype = {
	...
	getClusterAgent: function() {
		...
		if (this.clusterAgents.length == 0) {
			...
			// Throw an event that shows the MID server cluster has failed.
			this._throwClusterDownEvent();
	...
	_throwClusterDownEvent: function() {
		for (var i=0; i<this.clusters.length; i++)
			gs.eventQueue('mid_server.cluster.down', this.clusters[i], this.clusters[i].name + '', this.agent.name + '');
	},
