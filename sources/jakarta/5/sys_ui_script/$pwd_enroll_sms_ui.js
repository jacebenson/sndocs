...
function  addNewDevice(elem) {
	...
	var name = getFieldValue(currentForm, 'sysparm_device_name_' + verId, 
			'name_form_group_' + verId, 'retype_device_name_' + verId, gMsg.getMessage("Device name can not be empty"));  // I18N_OK 08-04-16
	if (name == "")
		return false;
	...
	if (useNotify == 'true') {
		countryCode = getFieldValue(currentForm, 'country_' + verId,
					'phone_form_group_' + verId, 'select_country_code_again_' + verId, gMsg.getMessage("Country code can not be empty"));  // I18N_OK 08-04-16
	...
	var phoneNumber = getFieldValue(currentForm, 'sysparm_phone_number_' + verId,
				'phone_form_group_' + verId, 'retype_phone_number_' + verId, gMsg.getMessage("Phone number can not be empty"));  // I18N_OK 08-04-16
	if (phoneNumber == "")
		return false;	
	...
	if (useNotify != 'true') {
		provider = getFieldValue(currentForm, 'service_provider_' + verId,
				'provider_form_group_' + verId, 'select_service_provider_again_' + verId, gMsg.getMessage("Provider can not be empty"));  // I18N_OK 08-04-16
		if (provider == "")
			return false;
	}
	/*
