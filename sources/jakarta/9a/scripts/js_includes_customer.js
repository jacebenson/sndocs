...
jQuery.noConflict();
$j = jQuery;
/*! RESOURCE: angular_wap_editor */
angular.module('angularapp', []);
angular.module('angularapp').controller("ItemController", ['$scope', '$location', '$anchorScroll', function($scope, $location, $anchorScroll) {
  $scope.jumpToLocation = function(key) {
    $location.hash(key);
    $anchorScroll();
  };
  $scope.arrowDown = "fa fa-arrow-down fa-4x";
  $scope.swapArrow = function() {
    alert("? " + $scope.arrowDown);
    if ($scope.arrowDown === "fa fa-arrow-down fa-4x")
      $scope.arrowDown = "fa fa-arrow-up fa-4x";
    else
      $scope.arrowDown = "fa fa-arrow-down fa-4x";
  };
  $scope.jumpToLocationArrow = function(key) {
  ...
  $scope.search = function(key) {
    alert("in search");
    $scope.jumpToLocation(key);
    var wap_listing = angular.element(document.querySelector('#wap-listing'));
    wap_listing.empty();
    var GR = new GlideRecord('cmdb_ci_wap_network');
    alert($scope.term);
    GR.addQuery('name', 'CONTAINS', $scope.term);
    GR.orderBy('name');
    GR.query(function() {
      alert("!");
    });
