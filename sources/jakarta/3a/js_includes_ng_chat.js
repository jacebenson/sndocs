  ...
  return {
    ...
    controller: function($scope, liveProfileID, messageService, messageFactory, uploadAttachmentService,
      ...
      $scope.isMessageTypeVisible = function() {
        return $scope.isDocumentGroup() && !hasPendingTransferToMe() && $scope.conversation.canSaveWorkNotes;
      };
      ...
      $scope.placeholderText = function() {
        if ($scope.isDocumentGroup() && $scope.conversation.canSaveWorkNotes) {
          if (isComment())
...
angular.module('sn.connect.resource').service('resourcePersister', function(
  ...
  function retrieve(conversationID) {
    ...
    $timeout(function() {
      snHttp.get(CONVERSATION_PATH + conversationID + "/resources").then(function(response) {
...
angular.module('sn.connect.queue').directive('snNonAgentClose', function(
      ...
      return {
        ...
        scope: {},
        controller: function($scope) {
            $scope.$on('connect.non_agent_conversation.close_prompt', function(event, c