...
var AJAXEmailClientCompleter = Class.create(AJAXTableCompleter, {
    ...
    if (!this.hasFocus) {
      ...
      if ((values.length == 1) ||
        ...
        if (this.renderItemTemplate) {
          ...
          s.innerHTML = s.innerHTML + '<span class="address" tabindex="0" onclick="addressOnClick(event, this)" style="white-space:nowrap;" value="' + values[0]['name'] + '" aria-label="' + values[0]['name'] + '">' + values[0]['label'] + ';</span> ';
        }
  ...
  select: function() {
    ...
    if (this.renderItemTemplate) {
      ...
      s.innerHTML = s.innerHTML + '<span class="address" tabindex="0" onclick="addressOnClick(event, this)" style="white-space:nowrap;" value="' + o['name'] + '" aria-label="' + o['label'] + '">' + o['label'] + ';</span> ';
    }
    this.clearDropDown();
    this._clearDisplayValue();
    this.currentDisplayValue = "";
...
function toggleGlideListIcons(id, performOnChange) {
  ...
  if ((id.startsWith("IO:") || id.startsWith("ni.VE") || id.startsWith("ni.QS")) &&
...
NOW.PageTiming = function() {
  ...
  function _pageTimingShow(info) {
    ...
    if (tot > 0) {
      ...
      if (window.performance) {
        timingGraph.attr("role", "button");
        timingGraph.attr("tabindex", "0");
        timingGraph.attr("aria-label", timingGraph.data("ariaLabel"));
        timingGraph.attr("aria-expanded", "false");
        timingGraph.one('click', function() {
          ...
          for (var i = 0; i < events.length; i++) {
            ...
            timingBreakdown.append($j('<tr><td aria-hidden="true" class="' + events[i][0] + '"></td><td>' + events[i][1] + '</td><td>' + startTime + 'ms</td><td>' + runTime + 'ms</td></tr>'));
          }
          timingBreakdown.insertAfter(this);
          timingGraph.attr("aria-disabled", "true");
          timingGraph.attr("aria-expanded", "true");
        });
      }
    }
  ...
  _announce: function(type, text) {
    if (this._announcer) {
      ...
      setTimeout(function() {
        ...
        that._announcer.style.display = 'none';
        that._announcer.style.display = 'inline';
      }, 500);
      setTimeout(function() {
...
! function(a, b) {
  "object" == typeof module && "object" == typeof module.exports ? module.exports = a.document ? b(a, !0) : function(a) {
    ...
    function pa(a, b) {
      return n.nodeName(a, "table") && n.nodeName(11 !=