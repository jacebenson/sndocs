...
calNS.compileCal = function compileCal() {
  ...
  if (window.NOW.filter_globals && window.NOW.filter_globals !== 'null' && window.NOW.filter_globals !== '[]') {
    var fiscalPeriods = JSON.parse(window.NOW.filter_globals);
    for (var i = 0; i < fiscalPeriods.length; i++) {
      var period = fiscalPeriods[i].concat(fiscalPeriods[i][2]);
      result.push(period);
    }
  }
