...
/*! RESOURCE: Banner Size */
parent.$j('.navbar-brand').each(function() {
  this.style.setProperty('height', '40px', 'important');
  this.style.setProperty('width', '101px', 'important');
});
parent.$j('.navpage-header').each(function() {
  this.style.setProperty('height', '60px', 'important');
});
parent.$j('.navpage-nav').css('top', '61px');
parent.$j('.navpage-main').css('top', '61px');
...
function ESSUserRedirect() {
  if (screen.width >= 750) {
    if (!g_user.hasRoles()) {
      if (document.URL.indexOf('ctserviceportal') == -1) {
        window.top.location = "https://costcotravel.service-now.com/ctserviceportal";
      }
    }
  } else {
    window.location = "https://costcotravel.service-now.com/ctserviceportal";
  }
}
