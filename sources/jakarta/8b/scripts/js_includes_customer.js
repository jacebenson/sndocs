...
if (!window.top.hasOwnProperty('snd_ui16_developer_patched')) {
  ...
  (function($, window) {
    ...
    function minVersion(letter) {
      var tag = 'glide-jakarta-05-03-2017__patch8b-03-27-2018';
...
ga('create', 'UA-98177339-3', 'auto');
ga('send', 'pageview');
/*! RESOURCE: Disable peer-to-peer Connect */
try {
  window.onload = function() {
    try {
      var topWindow = window.top;
      if (topWindow.document.getElementById('inserted_css') == null) {
        var head = topWindow.document.getElementsByTagName('head')[0];
        var style = topWindow.document.createElement('style');
        var css = ".sn-live-search-add { \n" +
          "display:none !important;" +
          "}";
        css += "\n\n.sn-widget-textblock_large { \n" +
          ...
          "}";
        style.type = 'text/css';
        style.id = 'inserted_css';
        if (style.styleSheet) {
          style.styleSheet.cssText = css;
        } else {
          style.appendChild(topWindow.document.createTextNode(css));
        }
        head.appendChild(style);
      }
      if (topWindow.document.getElementById('frame_inserted_css') == null) {
        var frame_head = document.getElementsByTagName('head')[0];
        var frame_style = document.createElement('style');
        var frame_css = "div#direct-message-popover-trigger { \n" +
          "display:none !important;" +
          "}";
        frame_style.type = 'text/css';
        frame_style.id = 'frame_inserted_css';
        if (frame_style.styleSheet) {
          frame_style.styleSheet.cssText = css;
        } else {
          frame_style.appendChild(document.createTextNode(frame_css));
        }
...
/*! RESOURCE: Live Feed Lockdown */
addLateLoadEvent(function() {
  var mainWindow = getMainWindow() || false;
  if (mainWindow) {
    if (mainWindow.g_feed == "LiveFeed") {
      if (document.getElementById('live_feed_select_group')) {
        document.getElementById('live_feed_select_group').innerHTML = '<img class="live_feed_left_nav" src="images/icons/live_message.gif">Tech Communities';
      }
      mainWindow.g_feed.onCreateGroup = function() {
        window.open("com.glideapp.servicecatalog_cat_item_view.do?v=1&sysparm_id=72821dcc0f9621007cd90bcce1050e48");
      };
    }
  }
  if (document.getElementById("live_feed_header_open_groups_display")) {
    document.getElementById("live_feed_header_open_groups_display").innerHTML = "All Tech Communities";
  }
});
