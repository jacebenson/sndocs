...
/*! RESOURCE: optionWorker */
var optionWorker = Class.create();
optionWorker.prototype = {
  initialize: function(field_name) {
    this.field_name = field_name;
    this.control = g_form.getControl(field_name);
    this.select_box = $j(this.control);
    if (this.control == undefined || this.control == null || this.select_box.length == 0) {
      this.control = null;
      this.select_box = null;
      console.error("optionWorker Error: field " + this.field_name + " does not exist");
    }
  },
  _getOption: function(option_value) {
    if (!this.select_box) {
      console.error("optionWorker Error: field " + this.field_name + " does not exist");
      return null;
    }
    var option = this.select_box.children("[value='" + option_value + "']");
    if (option.length == 0) {
      console.error("optionWorker Error: No option exists with value " + option_value + " in field " + this.field_name);
      return null;
    }
    return option;
  },
  disableOption: function(option_value) {
    option = this._getOption(option_value);
    if (!option)
      return;
    option.attr("disabled", "disabled");
  },
  disableOptions: function(option_values_array) {
    for (var i = 0; i < option_values_array.length; i++)
      this.disableOption(option_values_array[i]);
  },
  removeOption: function(option_value) {
    option = this._getOption(option_value);
    if (!option)
      return;
    option.remove();
  },
  removeOptions: function(option_values_array) {
    for (var i = 0; i < option_values_array.length; i++)
      this.removeOption(option_values_array[i]);
  },
  type: 'optionWorker'
}
/*! RESOURCE: GoogleAnalytics */
var _gaq = _gaq || [];
_gaq.push(['_setAccount', 'UA-53374744-1']);
...
function navFilterExtension(val, msg) {
  ...
  if (val.endsWith('.inc')) {
    ...
    record.addQuery("number", val.replace('.inc', ''));
    record.query();
    document.getElementById('gsft_main').src = "incident.do?sys_id=" + record.sys_id;
    restoreFilterText(msg);
    return true;
  }
  return false;
...
/*! RESOURCE: ss-instantticket-ctrl */
app.controller('MainCtrl', ['$scope', 'GlideAjax', function($scope, GlideAjax) {
  $scope.location = [];
  $scope.locationDisplay = [];
  $scope.locationString = '';
  $scope.showLocationDropdown = false;
  $scope.init = function() {};
  $scope.filterLocation = function(search) {
    $scope.locationDisplay = [];
    $scope.showLocationDropdown = true;
    for (var i = 0; i < $scope.location.length; i++) {
      if ($scope.location[i].name.indexOf(search) >= 0)
        $scope.locationDisplay.push($scope.location[i]);
    }
    $scope.$apply();
  };
  $scope.toggleLocationDropdown = function() {
    $scope.showLocationDropdown = !$scope.showLocationDropdown;
  };
  $scope.pushChangesToItem = function(option, which) {
    if (option.selected == true)
      $scope.locationString += option.name + ', ';
    else
      $scope.locationString = $scope.locationString.replace(option.name + ', ', "");
    $scope.locationString.split(' ').join(' ');
    g_form.setValue('location', $scope.locationString);
  };
  $scope.locationSearch = function() {
    var ajax = new GlideAjax("SSInstantTicketAJAX");
    ajax.addParam("sysparm_locationDisplay", $scope.locationDisplay);
    ajax.addParam("sysparm_location", $scope.location);
    ajax.addParam("sysparm_locationString", $scope.locationString);
    ajax.getJSON(function(response) {
      if (!response) {
        return;
      }
      $scope.locationDisplay = response;
      $scope.$apply();
    });
  };
  $scope.submitContact = function() {
    var ajax = new GlideAjax("SSInstantTicketAJAX");
    ajax.addParam("sysparm_name", $scope.name);
    ajax.addParam("sysparm_description", $scope.feedback);
    ajax.addParam("sysparm_location", $scope.location);
    ajax.addParam("sysparm_room", $scope.room);
    ajax.addParam("sysparm_urgency", $scope.urgency);
    ajax.getJSON(function(response) {
      if (!response) {
        return;
      }
      $scope.contact_number = response[0];
    });
