...
var GwtContextMenu = Class.create({
  ...
  display: function(e) {
    ...
    gActiveContext = this;
    showObject(menu);
    var firstItem = $(menu).select('.context_item')[0];
    if (firstItem)
      firstItem.focus();
  },
  hide: function() {
  ...
  hideAll: function() {
    ...
    while (m) {
      m.hide();
      var firingObject = m.getFiringObject();
      if (firingObject)
        firingObject.focus();
      m = m.getParent();
    }
  ...
  menuKeyDown: function(e) {
    ...
    switch (e.keyCode) {
      ...
      case 39:
        ...
        break;
      case 37:
        if (this.getParent().menu) {
          var parent = this.getParent().menu.select('[aria-controls="' + this.id + '"]');
          parent[0].focus();
          this._hideAllSubs(this.getParent().menu);
          handled = true;
        }
        break;
    }
  ...
  _focusNext: function(item) {
    ...
    while (nextSibling && nextSibling.classList.contains('context_item_hr'))
  ...
  _focusPrevious: function(item) {
    ...
    while (previousSibling && previousSibling.classList.contains('context_item_hr'))
...
function contextShow(e, tableName, timeoutValue, ttop, lleft, rright, firstElem) {
  ...
  if (getMenuByName(name)) {
    var contextMenu = getMenuByName(name).context;
    if ((!contextMenu.menu.style.left && !contextMenu.menu.style.top || contextMenu.menu.style.left == "0px" && contextMenu.menu.style.top == "0px") &&
      ttop == 0 && lleft == 0 && e.srcElement && e instanceof KeyboardEvent) {
      ttop = e.srcElement.offsetHeight;
      lleft = e.srcElement.offsetLeft;
    }
