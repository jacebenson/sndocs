...
addAfterPageLoadedEvent(function() {
  ...
  document.body.on('click', 'a[data-type="reference_popup"]', function(evt, element) {
    ...
    checkSave(table, form, ref, refKey, view);
  });
...
NOW.c14 = (function() {
  ...
  return {
    ...
    setShowRelated: function(fieldName, idx, name, select, filterMethod) {
...
function _createFilterSelect(width, multi, size) {
  ...
  s.setAttribute("aria-label", "Choose Value")
  if (width)
...
function updateFields(name, select, fOper, fValue, includeExtended, filter) {
  ...
  if (idx != -1)
    NOW.c14.setShowRelated(fieldName, idx, name, select, filter.filterFields.bind(filter));
...
function addOperators(td, type, dValue, isChoice, includeExtended, showDynamicReferenceOperator, filterClass, restrictI18NOpers) {
  ...
  s.setAttribute("aria-label", "Choose Operator");
