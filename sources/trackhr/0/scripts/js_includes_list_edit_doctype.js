...
GwtListEditRecordDecorations.Templates = {
  ...
  DELETE_ICON_DOCTYPE: new Template(
    '<i data-type="#{dataType}" data-list_id="#{listID}" class="list_delete_row list_decoration clsshort button icon-cross btn" title="#{title}" style="color:#FF402C" tabindex="0"></i>'
  ),
...
var GwtGridEdit = Class.create({
  ...
  _focus: function(e) {
    ...
    e.tabIndex = 0;
    if (this.rec < this._getMaxRow()) {
      var headerCell = this.tableController.getCellByNdx(0, this.col);
      if (!e.getAttribute('aria-label')) {
        var columnName = "";
        if (headerCell && headerCell.getElementsByClassName('column_head').length > 0)
          columnName = headerCell.getElementsByClassName('column_head')[0].innerText + ", ";
        e.setAttribute('aria-label', columnName + e.innerText);
      }
    }
    e.focus();
