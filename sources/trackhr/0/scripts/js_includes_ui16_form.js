    ...
    link: function(scope, element) {
      ...
      function updatePresence() {
        ...
        if (presences[id]) {
          ...
          if (element.prev().length > 0 && scope.profile) {
            status = status.replace(/ /g, "_");
            element.prev()[0].setAttribute('aria-label', scope.profile.userName + ' ' + presenceStatus[status]);
          }
        } else {
  ...
  return {
    ...
    controller: function($scope, $attrs, nowStream, snRecordPresence, snCustomEvent, userPreferences, $window, $q, $timeout, snMention, i18n) {
      ...
      function setPrimaryJournalField() {
        ...
        if ($scope.fields && visibleFields.indexOf(fieldName) == -1) {
