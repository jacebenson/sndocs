...
function glideList2InitEvents() {
  ...
  document.on('click', 'button[data-type="list_popup_v3"]', function(evt, element) {
    var gl = GlideList2.get(element.getAttribute('data-list_id'));
    var row = gl._getRowRecord(element);
    popListDiv(evt, row.target, row.sysId, gl.view, 600, true);
    evt.stop();
  });
  CustomEvent.observe('body_clicked', function(evt, element) {
    if ($j('.popup').length > 0 && !evt.target.closest('.popup') && evt.target.getAttribute('data-type') != "list2_popup") {
      lockPopup(evt);
      evt.stop();
    }
  });
