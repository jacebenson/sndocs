...
SourceControlAjax.prototype = Object.extendsObject(AbstractAjaxProcessor, {
	...
	start: function() {
		...
		var passwordThatMayNeedDecrypt = this.getParameter('sysparm_ajax_processor_user_input_password');
		var needsDecrypt = this.getParameter('sysparm_ajax_processor_needs_decrypt');
		var clearTextPassword = this._decryptPasswordIfNecessary(passwordThatMayNeedDecrypt, needsDecrypt, sys_repo_config_id);
	...
	/**
	 *  The UI will send the value of the password field. If it is from an existing record and the password hasn't
	 *  changed, this value will be a bunch of asterisks. So we need to get the password from the sys_repo_config
	 *  record and decrypt it, because the source control APIs require cleartext.
	 */
	_decryptPasswordIfNecessary: function(password, needsDecrypt, repoConfigId) {
		if (needsDecrypt != "true")
			return password;

		var gr = new GlideRecord('sys_repo_config');
		if (!gr.get(repoConfigId))
			return password;

		return new GlideEncrypter().decrypt(gr.password);
	},
