...
UpgradeClient.prototype = {
   ...
   process: function() {
   	  // prevent dist upgrade from being triggered during db upgrade
	  if (gs.isPaused()) {
          this.LOGGER.log('Dist upgrade not run because the system is paused. Dist upgrade might have already finished, or will be automatically triggered soon');
          return;
      }
	  if (this._shouldSkip()) {
	      return;
