...
SLATimeLineV2SNC.prototype = {
		...
		for (var i = 0; i < contractSlas.length; i++) {
			...
			for (var j = 0; j < relatedFieldsForContractSla.length; j++) {
				if (relatedFieldsForContractSla[j].indexOf('.') > -1 && !taskRefFieldValueMap.hasOwnProperty(relatedFieldsForContractSla[j])) {
					...
					var relatedFieldMap = {};
					var auditHistoryFieldPrefix = 'variables.';
					if (relatedFieldsForContractSla[j].startsWith(auditHistoryFieldPrefix)) {
						var gr = new GlideRecord(this.TABLE_ITEM_OPTION_NEW);
						if (gr.get(relatedFieldsForContractSla[j].substr(auditHistoryFieldPrefix.length))) {
							if (taskGr.variables.canRead()) {
								relatedFieldMap.read_allowed = true;
								relatedFieldMap.label = gr.getValue(this.ATTR_QUESTION_TEXT);
								for (var field in taskGr.variables) {
									if (gr.getValue(this.ATTR_NAME) == field) {
										relatedFieldMap.variable_name = field;
										relatedFieldMap.value = taskGr.variables[field];
										relatedFieldMap.display_value = taskGr.variables[field];
										break;
									}
								}
							}
								...
								relatedFieldMap.label = gr.getValue(this.ATTR_QUESTION_TEXT);
								for (var field in taskGr.variables) {
									if (gr.getValue(this.ATTR_NAM) == field) {
										relatedFieldMap.variable_name = field;
	...
	_getHistoricalTaskUpdates: function(taskGr) {
		...
		var walkedRecordGr;
		var hw = new sn_hw.HistoryWalker(taskGr.getRecordClassName(), taskGr.getUniqueValue());
