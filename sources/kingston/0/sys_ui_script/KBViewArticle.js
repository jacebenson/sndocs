...
var KbViewArticle = Class.create({
	...
	subscribeArticleHandler: function() {
		...
		ajax.getXML(function () {
			var msg = self.getL10N('You are now subscribed to {0}. You will be notified of any new comments or activity according to your notification preferences.');
...
function kbFeedbackDone(response) {
	...
	if(!result.getAttribute('id')){
		...
		window.scrollTo(0,0);
		return kbViewArticle.showErrorMessage('You have reached the daily limit for comments posted by a user.');
	}
