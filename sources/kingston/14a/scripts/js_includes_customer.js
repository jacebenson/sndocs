...
/*! RESOURCE: cmslite.functions */
var typingTimer;
var doneTypingInterval = 250;
$j(document).ready(function() {
  $j('#sysparm_search').keyup(function() {
    clearTimeout(typingTimer);
    typingTimer = setTimeout(doneTyping, doneTypingInterval);
  });
  $j('#sysparm_search').keydown(function() {
    clearTimeout(typingTimer);
  });

  function doneTyping() {
    OnScreenSearch(gel('sysparm_search'), '#searchResults');
  }
});

function OnScreenSearch(obj, container, scope) {
  var val = $j(obj).val();
  while (val.indexOf('  ') != -1) {
    val = val.replace('  ', ' ');
  }
  if (val.length > 2) {
    if ($j(window).width() > 100) {
      ...
      var ga = new GlideAjax('CMSAjaxSearch');
      ga.addParam('sysparm_name', 'OnScreenSearch');
      ga.addParam('sysparm_search', val);
      ga.addParam('sysparm_group', $j('#sysparm_search_group').val());
      ga.addParam('sysparm_scope', scope);
      ga.getXML(searchParse);
    }
  } else {
    $j(container).hide();
  }
}
$j(document).add(parent.document).mouseup(function(e) {
  var container = $j(e.target);
  if (container.attr("id") == 'home_search' || container.closest("#home_search").length > 0) {} else {
    $j("#searchResults").hide();
  }
});
var headHTML = document.getElementsByTagName('head')[0].innerHTML;
var faHeader = '<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css">';
document.getElementsByTagName('head')[0].innerHTML = headHTML + faHeader;

function dismissAlert(id) {
  $j.cookie("alert_" + id, true);
}

function checkAlertDisplay(id) {
  if ($j.cookie("alert_" + id) == "true") {
    if ($j("#alert_" + id).closest("div.alert-container").find("div.alert").length > 1) {
      $j("#alert_" + id).remove();
    } else {
      $j("#alert_" + id).closest("div.alert-container").remove();
    }
  } else {
    $j("#alert_" + id).show();
  }
...
function paginate(selector, items_per_page, visible_pages, paginate_container, prev, next, first, last) {
  var prevText = (prev != '') ? prev : null;
  var nextText = (next != '') ? next : null;
  var firstText = (first != '') ? first : null;
  var lastText = (last != '') ? last : null;
  var firstLastUse = (first != '' || last != '') ? true : false;
  var items = $j(selector);
  var parent = items.parent();
  parent.attr("pag", "true");
  parent.attr("pag_selector", selector);
  parent.attr("pag_items", items_per_page);
  parent.attr("pag_visible", visible_pages);
  parent.attr("pag_container", paginate_container);
  parent.attr("pag_prev", prev);
  parent.attr("pag_next", next);
  parent.attr("pag_first", first);
  parent.attr("pag_last", last);
  ...
  items.each(function() {
    ...
    $j(this).addClass("page_" + current_page);
    item_count++;
  ...
  paginateShowPage(selector, 1);
  $j(paginate_container).bootpag({
    total: current_page,
    page: 1,
    maxVisible: visible_pages,
    leaps: true,
    firstLastUse: firstLastUse,
    first: firstText,
    last: lastText,
    next: nextText,
    prev: prevText,
    ...
    paginateShowPage(selector, num);
  });
}

function paginateShowPage(selector, num) {
  $j(selector).hide();
  $j(selector + ".page_" + num).show();
}

function paginateDestroy(selector, container) {
  $j(container + " ul.bootpag").remove();
  $j(selector).each(function() {
    var css_class = $j(this).attr("class");
    css_class = css_class.replace("page_", "");
    $j(this).attr("class", css_class).show();
  });
}
/*! RESOURCE: updateOrderMonitorOptions */
function updateOrderMonitorOptions() {
  if (g_form.getValue('order_mobility_level') == 'stationary') {
    if (g_form.getBooleanValue('order_type_of_work_engineering') == 1 || g_form.getBooleanValue('order_type_of_work_developement') == 1) {
      enableOption('order_monitors', '1');
      enableOption('order_monitors', '1-2');
      enableOption('order_monitors', '2');
      enableOption('order_monitors', '2-2');
      enableOption('order_monitors', '3');
      enableOption('order_monitors', '3-2');
    } else {
      enableOption('order_monitors', '1');
      disableOption('order_monitors', '1-2');
      enableOption('order_monitors', '2');
      disableOption('order_monitors', '2-2');
      disableOption('order_monitors', '3');
      disableOption('order_monitors', '3-2');
    }
  } else if (g_form.getValue('order_mobility_level') == 'mobile') {
    if (g_form.getBooleanValue('order_type_of_work_engineering') == 1 || g_form.getBooleanValue('order_type_of_work_developement') == 1) {
      enableOption('order_monitors', '1');
      enableOption('order_monitors', '1-2');
      enableOption('order_monitors', '2');
  ...
  } else if (g_form.getValue('order_mobility_level') == 'traveling') {
    if (g_form.getBooleanValue('order_type_of_work_engineering') == 1 || g_form.getBooleanValue('order_type_of_work_developement') == 1) {
      enableOption('order_monitors', '1');
      disableOption('order_monitors', '1-2');
      enableOption('order_monitors', '2');
      ...
      enableOption('order_monitors', '1');
      disableOption('order_monitors', '1-2');
      disableOption('order_monitors', '2');
      disableOption('order_monitors', '2-2');
      disableOption('order_monitors', '3');
      disableOption('order_monitors', '3-2');
    }
