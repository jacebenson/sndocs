...
/*! RESOURCE: DisablePowerPaste */
function disablePopup() {
  var mces = tinyMCE.get();
  for (var i = 0; i < mces.length; i++) {
    var mce = mces[i];
    var settings = mce.settings;
    settings.powerpaste_word_import = "clean";
    settings.powerpaste_html_import = "clean";
    mce.editorManager.remove();
    mce.editorManager.init(settings);
  }
}
window.onload = function() {
  if (typeof tinyMCE !== 'undefined') {
    console.log('disablePopup: found instance');
    disablePopup();
  } else {
    console.log('disablePopup: no instance of TinyMCE');
  }
};
...
/*! RESOURCE: Jakarta fixes - TinyMCE tweaks */
$j(document).ready(function() {
  if (typeof window.tiny_html_editor === 'undefined' || !window.tiny_html_editor) return;
  var waitForTinyMCEInit = function() {
    if (window.waitForTinyMCEInitTryCount > 10)
    ...
    var fixTinyMCEBodyStyles = function(id) {
  ...
  window.waitForTinyMCEInitInterval = setInterval(waitForTinyMCEInit, 1500);
});
