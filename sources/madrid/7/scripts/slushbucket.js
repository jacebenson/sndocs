...
function onSlushKeyMove(evt, direction, sourceSelect, targetSelect, keepSourceLabel, unmovableSourceValues, keepTargetLabel) {
...
function expandFile(select, prefix) {
  ...
  if (option != null) {
    ...
    if (tableName == 'item_option_new' && listItemId) {
      if (isVariables(colName)) {
        reflistOpen(listItemId.id, 'cat_item', 'sc_cat_item');
        return;
      }
      if (isQuestions(colName)) {
        reflistOpen(listItemId.id, 'cat_item', 'sc_cat_item', null, null, null, 'sys_class_name=sc_cat_item_producer');
        return;
      }
    }
    if (tableName.length > 0) {
  ...
  if (items.length > 0) {
    ...
    if (shouldShowExtendedFields()) {
      ...
      for (var i = 0; i < sortedAddMe.length; i++) {
        ...
        var extensionPrefix = 'ref_';
        if (colName && colName != '')
          extensionPrefix = colName + '.ref_';
        appendSelectOption(select, extensionPrefix + field, document.createTextNode('extended field option'));
        var opt = select.options[select.options.length - 1];
        var innerHTML = label;
