...
/*! RESOURCE: /scripts/sp-tinymce/tinymce_listbox_a11y.min.js */
tinyMCE.ui.Factory.add("listboxa11y", function() {
  "use strict";
  var t = tinyMCE.ui.MenuButton,
    e = tinyMCE.ui.Menu,
    n = !1;
  return t.extend({
    init: function(t) {
      var e, u, a, i, s = this;
      s._super(t), t = s.settings, s._values = e = t.values, e && (void 0 !== t.value && function e(n) {
        for (var i = 0; i < n.length; i++) {
          if (u = n[i].selected || t.value === n[i].value) return a = a || n[i].text, s.state.set("value", n[i].value), !0;
          if (n[i].menu && e(n[i].menu)) return !0
        }
      }(e), !u && e.length > 0 && (a = e[0].text, s.state.set("value", e[0].value)), s.state.set("menu", e)), s.state.set("text", t.text || a), s.classes.add("listbox"), s.on("select", function(e) {
        var u = e.control;
        n = e, i && (e.lastControl = i), t.multiple ? u.active(!u.active()) : s.value(e.control.value()), i = u
      })
    },
    bindStates: function() {
      var t = this;
      return t.on("show", function(u) {
        var a, i;
        a = u.control, i = t.value(), a instanceof e && a.items().each(function(t) {
          t.hasMenus() || t.active(t.value() === i)
        ...
        var n = function t(e, n) {
          ...
          if (e)
            for (var a = 0; a < e.length; a++) {
              if (e[a].value === n) return e[a];
              if (e[a].menu && (u = t(e[a].menu, n))) return u
            }
        }(t.state.get("menu"), e.value);
        n ? t.text(n.text) : t.text(t.settings.text)
      }), t._super()
    }
  })
}());
