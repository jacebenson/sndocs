  ...
  var _this = {
    ...
    checkCompatabilityMode: function() {
      ...
      if (ieUserAgent.compatibilityMode) {
        alert('You are viewing the HelpNow portal with Internet Explorer\'s Compatibility Mode. For the best viewing experience, please turn off Compatibility Mode');
      }
    }
  };
...
addLateLoadEvent(function() {
  var parent = document.getElementById("banner_top_right");
  if (parent) {
    var portal_link = document.createElement("a");
    portal_link.innerHTML = "My Service Portal";
    portal_link.className = "ui11_switcher";
    portal_link.style.display = "block";
    portal_link.style.float = "left";
    parent.appendChild(portal_link);
    portal_link.addEventListener('click', function() {
      top.location = "customer_portal.do?sysparm_doctype=true";
    }, false);
  }
});
...
/*! RESOURCE: PortalUser_Redirect */
addLateLoadEvent(function() {
  try {
    function ajax_hasRoles() {
      var ajax = new GlideAjax('CustomerPortal');
      ajax.addParam('sysparm_name', 'userHasRoles');
      ajax.getXMLWait();
      return ajax.getAnswer() == 'true';
    }
    var userHasRoles = false;
    if (typeof g_user == "undefined") {
      userHasRoles = ajax_hasRoles();
    } else {
      userHasRoles = g_user.hasRoles();
    }
    var loc = window.location.toString();
    var safe_window = loc.indexOf('customer_portal.do') != -1 || loc.indexOf('export_data.do') != -1 || loc.indexOf('portal_login.do') != -1 || loc.indexOf('side_door.do') != -1 || loc.indexOf('login_cpw.do') != -1 || loc.indexOf('/events') != -1;
    if (!userHasRoles && !safe_window) {
      window.location = 'customer_portal.do?sysparm_doctype=true';
    }
  } catch (e) {}
});
...
/*! RESOURCE: Global window load event */
Event.observe(window, 'load', function() {
  if (/\/(catalog_)?home.do$/.test(location.pathname)) {
    g_load_functions.push(function() {
      var firstCatalogItemDivElmt = $$('[for_cat="7667526dfc50110026dc8c97cb22fa8a"]')[0];
      if (firstCatalogItemDivElmt) {
        var annotDiv = document.createElement('div');
        annotDiv.innerHTML = '<div style="background:#ffd;padding:8px;"><b>Note:</b> ' +
          'New Product Requests, Enhancements, and Defects should all be submitted via the ' +
          'Product Catalog.</div><hr style="border:0;height:1px;background:#ccc;" />';
        firstCatalogItemDivElmt.parentNode.insertBefore(annotDiv, firstCatalogItemDivElmt);
      }
    });
  }
});
...
/*! RESOURCE: FP - TeleBridge AjaxCalls */
function TeleBridgeResponse(response) {}

function makeCall(from, to, url, appSID, backupURL, callBackURL, sendDigits, ifMachine, timeout, record) {
  var TeleBridgeGA = new GlideAjax("TeleBridgeAjax");
  TeleBridgeGA.addParam('sysparm_name', 'makeCall');
  TeleBridgeGA.addParam('sysparm_from', from);
  TeleBridgeGA.addParam('sysparm_to', to);
  TeleBridgeGA.addParam('sysparm_url', url);
  TeleBridgeGA.addParam('sysparm_application_sid', appSID);
  TeleBridgeGA.addParam('sysparm_fallback_url', backupURL);
  TeleBridgeGA.addParam('sysparm_callback_url', callBackURL);
  TeleBridgeGA.addParam('sysparm_send_digits', sendDigits);
  TeleBridgeGA.addParam('sysparm_ifMachine', ifMachine);
  TeleBridgeGA.addParam('sysparm_timeout', timeout);
  TeleBridgeGA.addParam('sysparm_record', record);
  TeleBridgeGA.getXMLAnswer(TeleBridgeResponse);
}

function sendSMS(from, to, body, url, appSID) {
  var TeleBridgeGA = new GlideAjax("TeleBridgeAjax");
  TeleBridgeGA.addParam('sysparm_name', 'sendSMS');
  TeleBridgeGA.addParam('sysparm_from', from);
  TeleBridgeGA.addParam('sysparm_to', to);
  TeleBridgeGA.addParam('sysparm_body', body);
  TeleBridgeGA.addParam('sysparm_url', url);
  TeleBridgeGA.addParam('sysparm_application_sid', appSID);
  TeleBridgeGA.getXMLAnswer(TeleBridgeResponse);
}

function verifyUserPhoneNumber(userName, phoneNumber, callDelay) {
  return verifyPhoneNumber(userName + " Phone Number", phoneNumber, callDelay);
}

function verifyPhoneNumber(friendlyName, phoneNumber, callDelay) {
  showLoadingDialog();
  var TeleBridgeGA = new GlideAjax("TeleBridgeAjax");
  TeleBridgeGA.addParam('sysparm_name', 'verifyPhoneNumber');
  TeleBridgeGA.addParam('sysparm_friendly_name', friendlyName);
  TeleBridgeGA.addParam('sysparm_phone_number', phoneNumber);
  TeleBridgeGA.addParam('sysparm_call_delay', callDelay);
  TeleBridgeGA.getXMLWait();
  hideLoadingDialog();
  return TeleBridgeGA.getAnswer();
}
/*! RESOURCE: jquery.min */
(function(E, B) {
  function ka(a, b, d) {
