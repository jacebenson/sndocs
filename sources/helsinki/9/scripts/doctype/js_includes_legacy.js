...
function processColumns(tableDef, args) {
  ...
  if (items.length > 0) {
    ...
    if ((typeof showExtFields == 'undefined' && extPref != "false") ||
      ...
      var ajax = new GlideAjax('SysMetaExtendedFieldProcessor');
      ajax.addParam('table_name', tableName);
      ajax.getXML(function(response) {
        if (!response || !response.responseXML)
          return;
        sortedAddMe = [];
        if (response.responseXML.firstChild) {
          var data = response.responseXML.firstChild.getAttribute('answer');
          sortedAddMe = JSON.parse(data);
        }
        for (var i = 0; i < sortedAddMe.length; i++) {
          var efield = sortedAddMe[i];
          var field = efield.value;
          var label = efield.cl + " [" + efield.extlabel + "]";
          var ext = "ref_" + efield.ext;
          if (colName && colName != '')
            ext = colName + "." + ext;
          ...
          var innerHTML = label;
          if (efield.ref) {
            opt.reference = efield.reference;
            opt.doNotDelete = 'true';
            innerHTML = innerHTML + " [+]"
          }
          if (colLabel != '')
