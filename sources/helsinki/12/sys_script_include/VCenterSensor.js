...
VCenterSensor = {
	...
	handleVMs: function(type, dataMap, computedFieldsFunc) {
        var i, j, objectData, gr, template, virtualMacs, virtualMac, macs, sys_id, object,
				...
				if (!objectData.bios_uuid)
					...
					if (virtualMacs) {
						for (j = 0; j < virtualMacs.length; j++) {
							...
							macs.push('' + this.getXmlAttribute(virtualMac, 'mac'));
						}
					}
					...
					try {
						gr = VmwareVmCorrelator.processVmInstanceOrTemplate(objectData, macs, template, this.addDiscoveryCiStuff);
					} catch (e) {
