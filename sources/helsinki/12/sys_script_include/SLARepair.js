...
SLARepair.prototype = {
    ...
    repairBySysId: function(sysId, sourceTable, currentUser) {
        this.currentUser = currentUser || gs.getUserID();
    ...
    _repairFromTaskSLA: function() {
        ...
        var slaDefIds = [];
        var currentTaskId = this.taskSlaGr.getValue("task");
        if (this.baseTable != SLARepair.TASK)
            slaDefIds.push(taskSlaGr.getValue("sla"));
        do {
            this.tracker.incrementProgressValue();

            if (taskSlaGr.task != currentTaskId || !taskSlaGr.hasNext()) {
                var taskGr = taskSlaGr.task.getRefRecord();
                if (!taskGr.isValidRecord()) {
                    this.lu.logError("Task does not exist: " + taskSlaGr.getValue("task"));
                    continue;
                }

                this.tracker.updateDetailMessage(gs.getMessage("Processing SLA for {0}", taskGr.number));
                this._repairTaskSLAs(taskGr, slaDefIds);

                currentTaskId = taskSlaGr.getValue("task");
                slaDefIds.length = 0;
            }
            if (this.baseTable != SLARepair.TASK)
                slaDefIds.push(taskSlaGr.getValue("sla"));
    ...
    _preRepairForSLADef: function() {
        ...
        for ( var tableName in slaCnt) {
            ...
            if (this._isTableAudited(tableName)) {
                ...
                slaTot = slaTot + slaCnt[tableName];

                this.taskTables.push(tableName);
                this.slaDefs.concat(slaCnt[tableName].definitionIds);
            }
