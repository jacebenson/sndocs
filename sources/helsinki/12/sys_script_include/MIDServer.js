...
MIDServer.getDefault = function(schedule) {
    // if we have a MID server with the same name as our schedule, use it (what an odd behavior!)...
    var result = MIDServer.getByName(schedule.name);
    if (result)
        return;

    // as a last resort, just grab the first little ole MID server that comes along...
    var gr = new GlideRecord('ecc_agent');
    gr.query();
    return gr.next() ? new MIDServer(gr) : null;
};
