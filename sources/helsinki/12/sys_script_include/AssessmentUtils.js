...
AssessmentUtils.prototype = {
	...
	createAssessments: function(typeId, sourceId, userId, groupId) {
		(new SNC.AssessmentCreation()).createAssessments(typeId, sourceId, userId, false, groupId);
	},
	...
	getFilters : function(metricTypeId, table, groupField, condition, filterTable, filterCondition, showAllGroups) {
		...
		if (!groupIsReference) {
			...
			gr.addEncodedQuery(filterCondition);
			gr.groupBy(groupField);
			gr.query();
			while (gr.next()) {
				var value = gr[groupField] || '';
				uniqueGroups[value] = true;
			}
		} else if (showAllGroups == 'true' || showAllGroups == true) {
			var gr = new GlideRecord(filterTable);
			gr.addEncodedQuery(filterCondition);
			gr.query();
			uniqueGroups[''] = true;
			while (gr.next())
				uniqueGroups[gr.sys_id + ''] = true;
		} else {
			...
			gr.query();
			while (gr.next())
				acceptableValuesMap[gr.sys_id + ''] = true;
	...
	getAssessmentInstanceURL : function(/* String */instance) {
		...
		var url = instanceURL + '?uri=assessment_take2.do%3Fsysparm_assessable_type=' + type + '%26sysparm_assessable_sysid=' + instance;
	...
	getAssessmentTypeURL : function(/* String */type) {
		var url = gs.getProperty('glide.servlet.uri') + 'nav_to.do?uri=assessment_take2.do%3Fsysparm_assessable_type=' + type;
		return url;
	},
