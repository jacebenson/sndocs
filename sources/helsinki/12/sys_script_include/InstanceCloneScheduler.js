...
InstanceCloneScheduler.prototype = {
    ...
    notifyServer: function(cloneRecord){
        ...
        s.setStringParameter('cluster_node', this._getSystemId(cloneRecord.cluster_node));
        ...
        s.setStringParameter('source_instance_name', gs.getProperty('instance_name') || sourceName);
        ...
        // version details
        s.setStringParameter('source_version', this._formatVersion(gs.getProperty('glide.war')));
        s.setStringParameter('target_version', this._formatVersion(targetInstance.war_version));
        ...
        var targetJDBCUrl = targetInstance.database_url.replace("localhost", primaryIP);
        var targetDBName = targetInstance.database_name;
        var standbyJDBCUrl = standbyInstance == null ? "" : standbyInstance.database_url.replace("localhost", primaryIP);
        var standbyDBName = standbyInstance == null ? "" : standbyInstance.database_name;
        ...
        s.setStringParameter('standby_db_server', this._formatDBServer(standbyJDBCUrl));    
	   ...
	   s.setRequestHeader("Authorization", "Clone " + gs.getProperty('instance_id') + ":" + primaryIP);
        ...
        if (s.httpStatus != 200)
            ...
            var dateChanged = cloneRecord.scheduled.getDisplayValue() != scheduled.getDisplayValue();
            ...
            if ( stateChanged ) {
                gs.log("InstanceClone", "Schedule Server changed state from " + oldState + " to " + state + " (" + description + ")");
            ...
            if ( dateChanged ) {
                gs.log("InstanceClone", "Schedule Server changed date from " + oldScheduled.getDisplayValue() + " to " + scheduled.getDisplayValue() + " (" + description + ")");
            ...
            if ( nodeChanged && this._setClusterNode(cloneRecord, newClusterNode) ) {
                update = true;
                gs.log("InstanceClone", "Schedule Server changed cluster node from " + oldClusterNode + " to " + newClusterNode + " (" + description + ")");
            }

            if ( standbyChanged && this._setStandbySource(cloneRecord, newStandbyJDBCUrl, newStandbyDBName) ) {
                update = true;
                gs.log("InstanceClone", "Schedule Server changed standby db from " + standbyJDBCUrl + "." + standbyDBName + " to " + newStandbyJDBCUrl + "." + newStandbyDBName + " (" + description + ")");
            }
    ...
    scheduleNotifyServer: function(cloneRecord) {
        ...
        schedule.script = "new InstanceCloneScheduler()._notifyServerByCloneId('" + cloneRecord.sys_id + "');";
    ...
    _setClusterNode: function(cloneRecord, clusterNode) {
      ...
      gr.query();
      if ( gr.next() && (gs.nil(cloneRecord.cluster_node) || cloneRecord.cluster_node != gr.sys_id) ) {
    ...
    _setStandbySource: function(cloneRecord, jdbcUrl, dbName) {
      ...
      try {
        var instance = new SncCloneInstance(dbName, user, encryptedPassword, jdbcUrl, rdbms, tablespace);

        // verify that the database exists
        if (!instance.instanceDatabaseExists()) {
            SncCloneLogger.log("InstanceClone", cloneRecord.sys_id, null, /*error*/2, "Source cannot be cloned from: database does not exist");
            return false;
        }

        // verify that the alternative source can be cloned from
        if (!instance.canCloneFrom()) {
            SncCloneLogger.log("InstanceClone", cloneRecord.sys_id, null, /*error*/2, "Source cannot be cloned from: instance_id does not match primary database (it appears to be a different database)");
            return false;
        }
      } catch(e) {
          SncCloneLogger.log("InstanceClone", cloneRecord.sys_id, null, /*error*/2, "Source cannot be cloned from: unable to connect to database and verify instance_id: " + e.toString());
          return false;
      }
    ...
    _shouldSkip: function(sourceName, targetName) {
		...
		var isDeveloper = GlideUtil.isDeveloperInstance() && !GlideUtil.isProductionInstance();
	    var isLocalHost = sourceName.indexOf("localhost") > -1 || targetName.indexOf("localhost") > -1;
		...
		if (url.indexOf(productionUrl) > -1 && (isDeveloper || isLocalHost)) {
			answer = true;
			gs.print("InstanceCloneScheduler - this is a developer or localhost instance, production scheduling service is not supported: to resolve, change property glide.db.clone_server_url");
		}
