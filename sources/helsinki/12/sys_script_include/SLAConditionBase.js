...
SLAConditionBase.prototype = {
	...
	cancel: function() {
		...
		if (this._useContract())
			return cancelMatch || this._cancelContractSLA();
	...
	_cancelContractSLA: function() {
		...
		gr.addActiveQuery();
		gr.addQuery('contract', this.task.contract);
		gr.addQuery('contract_sla',this.sla.sys_id);
		gr.query();
		
		// Contract can have multiple sla's
		if (gr.hasNext()) {
			return false;
		}
	...
	_conditionMatches: function(condition) {
		...
		var value;
		try {
			value = GlideFilter.checkRecord(this.task, condition);
		}
		catch (e) {
	...
	// checks if Contracts SLA Plugin is enabled for the current task type
	_useContract: function() {
		var contractTables = gs.getProperty('com.snc.sla.contract.tables', '');
		
		if(contractTables) {
			var arr = contractTables.replaceAll(" ", "").split(",");
			return new ArrayUtil().contains(arr, this.task.getRecordClassName());
		}
