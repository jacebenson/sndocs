	...
	_checkSchedule: function(ciType, cmdb_ci, changeGR) {
		...
		if (!overlaps.isEmpty()) {
			...
			if (overlapSpan.getStart().equals(this.startSDT) && overlapSpan.getEnd().equals(this.endSDT))
		      ...
		      if (ciType == "affected")
			   gs.addInfoMessage("Change planned times fall outside the maintenance schedule for affected CI: " + 
				cmdb_ci.getDisplayValue());
		      else 
			   gs.addInfoMessage("Change planned times fall outside the maintenance schedule for CI: " + 
				cmdb_ci.getDisplayValue());
                   }
                }
   		return false;
