...
ERROR_TITLE = 'ERROR';
ERROR_NO_PAGE_ID = 'One of the required fields was not specified. Make sure "sysparm_timeline_page_id" is specified.';
ERROR_INTERACTIVE = 'One of the required parameters was not properly specified';
ERROR_SECURITY = 'Modification of this record is not allowed';
ERROR_INVALID_PAGE_ID = 'Unable to find a timeline page with the specified ID';
ERROR_INVALID_SUB_ID = 'Unable to find a timeline page sub item with the specified ID';
...
TimelineGeneratorSchedulePage.prototype = Object.extendsObject(AbstractTimelineSchedulePage, {
	...
	_setStatusError: function(code) {
		switch(code) {
			...
			return this.setStatusError(gs.getMessage(ERROR_TITLE), gs.getMessage(ERROR_NO_PAGE_ID));
			case this.ERROR_CODE_INVALID_ID:
			return this.setStatusError(gs.getMessage(ERROR_TITLE), gs.getMessage(ERROR_INVALID_PAGE_ID));
			case this.ERROR_SUB_INVALID_ID:
			return this.setStatusError(gs.getMessage(ERROR_TITLE), gs.getMessage(ERROR_INVALID_SUB_ID));
		}
	},
		...
		while (gr.next()) {
			...
			if (result.show_tooltips == true) {
				...
				for (var j = 0, k = cols.length; j < k; j++) {
					...
					strText += '<strong>' + label + '</strong>: ' + displayValue + '<br />';
				}
	...
	elementMoveX : function(spanId, newStartDateTimeMs) {
		if (!spanId || spanId == null || spanId == '')
			return this.setStatusError(gs.getMessage(ERROR_TITLE), gs.getMessage(ERROR_INTERACTIVE));
		...
		if (gr.next()) {
			...
			gr.setValue(result.end_date_field, gdtEnd);
			if (!gr.update())
				return this.setStatusError(gs.getMessage(ERROR_TITLE), gs.getMessage(ERROR_SECURITY));
