...
expandRole: function(/* GlideRecord */ sys_user_has_role) {
   if (this.shouldUseInhCount())
 	  return;
   var topLevel = false;
   if (typeof ROLE_HASH == 'undefined' || ROLE_HASH == null) {
      topLevel = true;
      ROLE_HASH = new Object();
   }
   // key is  role id  + user record id so we know if we have added the role for target user.
   var user = sys_user_has_role.user;
   var key = sys_user_has_role.role + '_' + user.sys_id;
   ROLE_HASH[key] = true;
   var expansion = new GlideRecord('sys_user_role_contains');
   expansion.addQuery('role', sys_user_has_role.role);
   expansion.query();
   while (expansion.next()) {
      var childkey = expansion.contains + '_' + user.sys_id;
      if (ROLE_HASH[childkey]) {
         gs.log("Role " + expansion.contains + " has been expanded for user " + user.name);
         continue;
      }
      var msgArray = new Array();
