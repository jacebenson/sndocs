(function(){
	...
	function deleteBackupImages(tableSysId) {
		...
		gr.query();
		while(gr.next()) {
			grThumb = new GlideRecord('sys_attachment');
			grThumb.addQuery('file_name', 'CONTAINS', gr.getUniqueValue());
			grThumb.query();
			while(grThumb.next()){
				attachment.deleteAttachment(grThumb.getUniqueValue());
			}
			gr.print('deleting attachment and thumbnails for: ' + gr.getUniqueValue());
			attachment.deleteAttachment(gr.getUniqueValue());
		}
	}
