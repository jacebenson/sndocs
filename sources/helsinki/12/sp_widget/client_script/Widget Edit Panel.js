function ($scope, spUtil, $location, $timeout, $window, $rootScope, $uibModal, spPreference, $compile) {
	...
	c.example = function(id) {
		$window.location.href = '?id=widget_editor&spa=1&sys_id=' + id;
	}
	...
	c.closeProvider = function(provider) {
		provider.show = false;
		$rootScope.$broadcast('$sp.wep-providers', 'close', c.data.sys_id);
	}
	...
	$scope.$on('field.change', function(evt, params) {
		...
		c.server.update().then(function() {
			...
			$timeout(function() {
				...
				setReadOnly();
				setPageTitle();
				$rootScope.$broadcast('$sp.wep-providers', 'clear');
				$rootScope.$broadcast('$sp.wep-providers', 'load');
	...
	c.save = function() {
		...
		$rootScope.$broadcast('$sp.wep-providers', 'save', c.data.sys_id);
	...
	c.editOptionSchema = function(event){
		...
		myModal.options.modalBodyClass = "wrapper";
