...
function addBoxFields(tableName, fValue, isSort, isGroup, sort) {
  setup(tableName);
  var s = _createFilterSelect();
  if (!isSort)
    s.onchange = function() {
  ...
  addFirstLevelFieldsWithoutRef(s, tableName, null, fValue, boxFilter);
  return s;
}
...
function addNonChoiceNumericFields(tableName, fValue, isSort, isGroup, sort) {
  setup(tableName);
  var s = _createFilterSelect();
  if (!isSort)
    s.onchange = function() {
  ...
  addFirstLevelFieldsWithoutRef(s, tableName, null, fValue, nonChoiceNumericFilter);
  return s;
}
...
function stackFilter(item) {
  ...
  if (choice == 1 || choice == 3)
    return true;
  if (item.getNamedAttribute("can_stack") == 'true')
    return true;
  return false;
}
...
function setSelect2Filters() {
  if ($("isSelect2") && $j('#gcond_filters td#field select').select2)
    $j('#gcond_filters td#field select').select2();
  if ($("isSelect2") && $j('#gcond_sort_order td#field select').select2)
    $j('#gcond_sort_order td#field select').select2();
}

function addSortField(name) {
  addSortSpec(name);
  setSelect2Filters();
}