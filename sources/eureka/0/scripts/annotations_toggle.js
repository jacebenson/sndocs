...
SN.formAnnotations = {
  ...
  init: function() {
    ...
    SN.formAnnotations.annotations = jQuery(selectors.join(','));
    if (SN.formAnnotations.annotations.length) {
      jQuery("#header_toggle_annotations").show().click(SN.formAnnotations.toggle);
      if (!SN.formAnnotations.preference && SN.formAnnotations.infoPreference) {
        SN.formAnnotations.addHiddenAnnotationMessage();
        jQuery("#info_toggle_annotations").show().click(SN.formAnnotations.toggleFromInfoMsg);
      }
    }
    if (SN.formAnnotations.preference)
      SN.formAnnotations.show();
  ...
  addHiddenAnnotationMessage: function() {
    ...
    msg += ' <span id="info_toggle_annotations" tabindex="0" class="icon-button icon-help sn-cloak" title="Toggle annotations on / off" style="display: inline-block;color:#678;cursor:pointer;font-size:1.4em;vertical-align:bottom;"></span> ';
