var GwtNavFilter = {
  ...
  _matchModules: function() {
    ...
    for (var i = 0, n = modules.length; i < n; i++) {
      ...
      if (this.favoritesOnly) {
        if (jQuery(module).children("span.icon-star").length !== 1)
          continue;
      }
  ...
  _showModulesForApp: function(span, displayed) {
    var trs = span.getElementsByTagName("div");
  ...
  _saveUserPreferences: function() {
    ...
    for (var i = 0, n = modules.length; i < n; i++) {
      ...
      if (type != 'SEPARATOR')
        continue;
      var id = module.getAttribute('moduleid');
      var tr = module.nextSibling;
      if (!tr)
        continue;
      var span = tr.firstChild.firstChild;
      if (!span)
        continue;
  ...
  _hideAppAndModules: function(id) {
    ...
    $j(app).find('div').each(function(index, elem) {
