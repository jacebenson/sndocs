var GlideViewManager = Class.create({
  ...
  _addViews: function(xml) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var selected = thisView == v.toLowerCase();
  ...
  _buildNewSelectedList: function(xml) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (value.startsWith(".annotation.")) {
        o.setAttribute("annotationtext", item.getAttribute("annotationtext"));
        o.setAttribute("showannotation", item.getAttribute("showannotation"));
        o.setAttribute("annotationtype", item.getAttribute("annotationtype"));
        o.setAttribute("annotationid", item.getAttribute("annotationid"));
      }
