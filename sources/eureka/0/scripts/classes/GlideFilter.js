...
GlideFilter.prototype = {
  ...
  initialize: function(name, query, fDiv, runnable, synchronous) {
    ...
    this.setOptionsFromParmsElement(name);
  },
...
GlideFilterSection.prototype = {
  ...
  addSortHeader: function() {
    ...
    $(tr).addClassName('sn-filter-order-message');
    var td = cel('td', tr);
    td.style.verticalAlign = "top";
    td.style.width = "100%";
    td.innerHTML = "<hr></hr><span>" + this.answer['Order results by the following fields'] + "</span>";
  },
  ...
  _addHRTR: function(first) {
    ...
    td.style.verticalAlign = "top";
    td.style.width = "100%";
    $(td).addClassName('sn_multiple_conditions');
    this.sectionSep = td;
    if (first)
  ...
  _addMessageTR: function(first) {
    var tr = cel('tr', this.tbody);
    $(tr).addClassName('sn-filter-section-message');
    var td = cel('td', tr);
    td.style.verticalAlign = "top";
    td.style.width = "100%";
    this.sectionSepMessage = td;
    if (first)
      td.innerHTML = getMessage("All of these conditions must be met");
    else
      td.innerHTML = getMessage("OR all of these conditions must be met");
  },
  setFirst: function(length) {
    this.sectionSepMessage.innerHTML = getMessage("All of these conditions must be met");
  },
...
GlideConditionRow.prototype = {
  AND_IMAGE: "images/and_text.pngx",
  OR_IMAGE: "images/or_text.pngx",
  DELETE_IMAGE: "images/closex.gifx",
  DELETE_HOVER_IMAGE: 'images/closex_hover.gifx',
  TD_VERTICAL_ALIGN: 'top',
  ...
  setAsRunRow: function(field) {
    ...
    td.innerHTML = GlideFilterUtil.buildRunButton(runCode);
  },
  ...
  addTD: function(row, queryID) {
    ...
    td.valign = "top";
    td.style.verticalAlign = this.TD_VERTICAL_ALIGN;
    return td;
  },
  ...
  addLeftButtons: function() {
    if (this.wantOr) {
      ...
      var orOnClick = "newSubRow(this,'" + fDiv + "')";
  ...
  getAndButtonHTML: function(onClick) {
    return "<a tabindex='0' onclick=" + onClick + "><IMG height='16' width='16' src='" + this.AND_IMAGE + "' title='" + this.answer['Add AND Condition'] +
      "' alt='" + this.answer['Add AND Condition'] + "' BORDER='0' class='filerTableAction' /></a>";
  },
  getOrButtonHTML: function(onClick) {
    return "<a tabindex='0' onclick=" + onClick + "><IMG height='16' width='16' src='" + this.OR_IMAGE + "' title='" + this.answer['Add OR Condition'] +
      "' alt='" + this.answer['Add OR Condition'] + "' BORDER='0' class='filerTableAction' /></a>";
  },
  getDeleteButtonHTML: function(id, onClick) {
    return '<a tabindex="0" onclick=' + onClick + '><IMG id="img_' + id + '" src="' + this.DELETE_IMAGE + '" alt="' + this.answer['Delete'] + '" title="' + this.answer['Delete'] +
      '" BORDER="0" class="filerTableAction" /></a>';
  },
  makeFirst: function() {
    var tdMessage = this.tdAndOrText;
    tdMessage.innerHTML = '';
...
var GlideFilterUtil = {};
GlideFilterUtil.buildRunButton = function(f) {
  var m = new GwtMessage();
  return '<a' +
    '  tabindex="0" onclick="' + f + '" title="' + m.getMessage('Run Filter') + '">' +
    '   <img height="16" width="16" border="0" src="images/run.pngx" />' + m.getMessage('Run') +
    '</a>';
}
...
function createCondFilter(tname, query, fieldName) {
  noOps = false;
  noSort = false;
  noGroup = true;
