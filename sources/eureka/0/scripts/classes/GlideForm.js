...
var GlideForm = Class.create({
  ...
  getSections: function() {
    ...
    for (var i = 0; i < spans.length; i++) {
      var span = spans[i];
      if (!span.id.startsWith("section."))
        continue;
      if (span.getAttribute('data-header-only') == 'true')
        continue;
      sections.push(span);
    }
    return sections;
  ...
  showRelatedList: function(listTableName) {
    var relatedListID = this._getRelatedListID(listTableName);
    g_tabs2List.showTabByID(relatedListID);
    if (!g_tabs2List.isActivated())
      show(relatedListID);
  },
  hideRelatedList: function(listTableName) {
    var relatedListID = this._getRelatedListID(listTableName);
    g_tabs2List.hideTabByID(relatedListID);
    if (!g_tabs2List.isActivated())
      hide(relatedListID);
  },
