...
var AJAXReferenceCompleter = Class.create(AJAXCompleter, {
  ...
  ajaxRequest: function() {
    ...
    if (s.length == 0 && !this.isDoctype()) {
      this.log("ajaxRequest did not execute due to empty query");
      this.clearDropDown();
  ...
  _processBlurValue: function(values, recents) {
    ...
    values = values || [];
    recents = recents || [];
    if (values.length + recents.length === 0) {
      this.setInvalid();
      return;
    }
    ...
    if (targetLabel) {
      this.log("setting value without focus to " + targetLabel + "->" + targetValue);
  ...
  addSysParms: function() {
    ...
    if (this.refQual != "" && typeof this.refQual != "undefined")
  ...
  _setUpDocMouseDown: function() {
    if (this.isTablet()) {
      this.blurPause = true;
      this.log("document mousedown event configured");
  ...
  onDocMouseDown: function(evt) {
    this.log("document mousedown " + evt.target);
    if (evt.target == this.element) {
      this.log("ignoring document mousedown as we clicked the element");
      return;
    }
    this.log("document mousedown setting blurPause to false");
    this.blurPause = false;
  },
  onBlurEvent: function(evt) {
    this.log("blur event");
    if (this.isTablet() && this.blurPause == true) {
      this.log("tablet pause on blur event");
      setTimeout(this.onBlur.bind(this), 4000);
    } else
      this.onBlur();
  },
  onBlur: function() {
    this.log("onBlur function");
    if (this._boundOnDocMousedown) {
      this.log("stop observing mouse up");
      ...
      if (!this.isFilterUsingContains) {
        ...
        if (this.isExactMatch()) {
          if (this.oneMatchSelects != false) {
            this.log("one match select on blur");
