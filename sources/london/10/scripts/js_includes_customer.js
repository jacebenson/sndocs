...
N.sanitize = function(s, replaceChar) {
  if (typeof replaceChar == 'undefined') {
    replaceChar = "_";
  }
  var n = s + '';
  n = n.toLowerCase();
  n = n.replace(/[^\w.]/g, replaceChar);
  return n;
};
N.dv = function(fieldName) {
  if (this.nil(g_form)) {
    return '';
  }
  var displayBox = g_form.getDisplayBox(fieldName);
  if (this.nil(displayBox)) {
    return '';
  }
  return displayBox.value + '';
};
N.isChanged = function(fieldName) {
  if (this.isNil(g_form) || this.isNil(fieldName)) {
    return false;
  }
  var el = $(g_form.getTableName() + "." + fieldName);
  if (this.isNil(el)) {
    return false;
  }
  return g_form.changedFieldsFilter(el);
};
N.hasChanged = function(fieldName) {
  return this.isChanged(fieldName);
};
N.changes = function(fieldName) {
  return this.isChanged(fieldName);
};
N.nil = function(given) {
  try {
    if (typeof given == 'undefined') {
      return true;
    } else if (given === null || given == '' || given == 0) {
      return true;
    }
  } catch (e) {
    return true;
...
N.getLabel = function(fieldName) {
  try {
    var ih = g_form.getLabel(fieldName).innerHTML + '';
    return ih.substring(0, ih.length - 1);
  } catch (e) {
    this.l("N.getLabel encountered an error: " + e, "N.getLabel");
  }
  return '';
};
...
N.isAfterToday = function(start) {
  return this.isAfterDate(new Date(), start);
};
...
N.isAfter = function(start, end) {
  ...
  var startMS = this.dff(start);
  if (startMS == 0 && typeof g_form != 'undefined') {
    startMS = this.dff(g_form.getValue(start));
  }
  var endMS;
  if (this.nil(end)) {
    ...
    endMS = this.dff(end);
    if (endMS == 0 && typeof g_form != 'undefined') {
      endMS = this.dff(g_form.getValue(end));
    }
  }
  if (startMS < endMS) {
    return true;
...
N.isAfterDate = function(start, end) {
  ...
  var startMS = this.dff(start, true);
  if (startMS.getTime() == 0 && typeof g_form != 'undefined') {
    startMS = this.dff(g_form.getValue(start), true);
  }
  startMS = startMS.setHours(23, 59, 59, 999);
  var endMS;
  if (this.nil(end)) {
    endMS = new Date();
  } else {
    endMS = this.dff(end, true);
    if (endMS == 0 && typeof g_form != 'undefined') {
      endMS = this.dff(g_form.getValue(end), true);
    }
  }
...
N.isBeforeDate = function(start, end) {
  if (this.nil(start)) {
    this.l("N.isBeforeDate called with no start");
    return;
  }
  var startMS = this.dff(start, true);
  if (startMS.getTime() == 0 && typeof g_form != 'undefined') {
  ...
  if (this.nil(end)) {
    ...
    endMS = this.dff(end, true);
    if (endMS.getTime() == 0 && typeof g_form != 'undefined') {
      endMS = this.dff(g_form.getValue(end), true);
    }
  }
  endMS = endMS.setHours(0, 0, 0, 0);
  return startMS < endMS;
};
N.dff = function(d, f, obj) {
  ...
  if (arguments.length == 3) {
    d = getDateFromFormat(d, f);
  } else if (this.nil(d.getTime)) {
    if (/ /.test(d)) {
...
N.showSection = function(sectName, bool) {
  sectName = sectName.toLowerCase().replace(' ', '_');
  g_form.setSectionDisplay(sectName, bool);
};
N.hideSection = function(sectionName) {
  this._getSection(sectionName).hide();
  this._getSectHead(sectionName).hide();
};
N._getSection = function(sectionName) {
  return $$('span[tab_caption="' + sectionName + '"]')[0].select('span[id*=section.]')[0];
};
N.showRelatedPopup = function(refFieldElID, title, table, column, value) {
  var s = refFieldElID.split('.');
  title = (this.notNil(title)) ? title : "Related records";
  table = (this.notNil(table)) ? table : $('sys_display.' + refFieldID).readAttribute('data-table') + '';
  column = (this.notNil(column)) ? column : s[1] + '';
  value = (this.notNiil(value)) ? value : g_form.getValue(s[1]);
...
N.selectRelatedList = function(title) {
...
N.getParameterValue = function(parameterName) {
...
N.addChoiceValue = function(field, displayValue, value, position) {
  if (field && displayValue && value) {
    position = parseInt(position);
    var getElement = g_form.getControl(field);
    getElement.insertBefore(new Option(displayValue, value), getElement.options[isNaN(position) ? getElement.length : position]);
  }
};
...
function addUnimpersonateButton() {
  try {
    if ($('user_info_dropdown').value != '') {
      $('user_info_dropdown').insert({
        after: '<span id="unimpersonate_span" style="visibility: visible;"><img onclick="unimpersonateMe();" title="End impersonation" src="images/icons/stop.gifx" style="cursor:pointer;cursor:hand;margin-top:5px;margin-left:-3px" height="22" width="22"></img></span>'
      });
    }
  } catch (e) {}
}
...
function unimpersonateMe() {
  top.location.href = 'ui_page_process.do?sys_id=b071b5dc0a0a0a7900846d21db8e4db6&sys_user=' + $('user_info_dropdown').value;
}
...
window.onload = function() {
  ...
  window.top.document.title = myTitle;
};

function u_getPageType(filename) {
  if (filename.indexOf('sys') == 0) {
...
/*! RESOURCE: PutPrintIconBackWhereItBelongs */
...
/*! RESOURCE: HU - Remove TinyMCE ToolTip */
window.setTimeout(hideHTMLHelpIcon, 1200);

function hideHTMLHelpIcon() {
  tinymce.editors.forEach(function(editor) {
    var row = editor.editorContainer.parentElement.getElementsByClassName('row');
    jQuery(row[0].children[0].children[0]).remove();
    jQuery(row[0].children[0]).remove();
  });
}
/*! RESOURCE: GlobalSearchVisByDefault */
addLoadEvent(SearchBarVis);
try {
  top.$j('#sysparm_search').css('width', '250px');
} catch (ex) {
  jslog(ex);
}

function SearchBarVis() {
  try {
    top.$j('#sysparm_search').attr('class', 'form-control form-control-search focus');
    top.$j('#sysparm_search').css('opacity', '1');
    top.$j('#sysparm_search').css('border', '1px solid');
    top.$j('#sysparm_search').css('box-shadow', 'none');
    top.$j('#sysparm_search').css('background', '#fff');
    top.$j('#sysparm_search').css('color', '#000');
    top.$j('#sysparm_search').css('border-color', '#fff');
  } catch (ex) {
    jslog(ex);
  }
}
...
/*! RESOURCE: unimpersonate_ui16 */
document.addEventListener("DOMContentLoaded", function(event) {
  if (g_user.userName.toLowerCase() != 'guest') {
    var imper_id = _retrieveImpersonaterID();
    if (imper_id) {
      ...
      if (theURL.indexOf('nav_to.do?') != -1 || theURL.indexOf('navpage.do') != -1 || theURL == baseURL) {
        ...
        if (jQuery(theLinkElement).length == 0) {
          ...
          var Unimpersonate = "<a href='/ui_page_process.do?sys_id=b071b5dc0a0a0a7900846d21db8e4db6&sys_user=" + imper_id + "'id='Unimpersonate' class='headerlink' style='color: inherit;text-decoration: underline;'>Unimpersonate</a>";
          var theHeaderElement = getTopWindow().document.querySelector(".nav.navbar-right");
          jQuery(linkDivOpen + Unimpersonate + linkDivClose).insertAfter(theHeaderElement);
        }
      }
    } else {
      return;
  ...
  function _retrieveImpersonaterID() {
    var returnObj;
    var client = new XMLHttpRequest();
    client.open("get", "/impersonation_status.do", false);
    ...
    if (returnObj.impersonating) {
      return returnObj.impersonater_id;
    } else {
      return false;
    }
  }
