  ...
  return {
    ...
    link: function(scope, element, attr) {
      ...
      if (scope.field.type == "integer" || scope.field.type == "decimal")
        scope.field.stagedValue = formatNumber(scope.field.stagedValue);
    }
  }
});;
...
angular.module('sn.$sp').directive('spTinymceEditor', function(getTemplateUrl, snAttachmentHandler, $timeout, i18n, spAriaUtil, $rootScope) {
  return {
    ...
    controller: function($scope, $element, $attrs) {
      ...
      $scope.textareaId = $scope.textId || 'ui-tinymce-' + (new Date().valueOf());
...
angular.module('sn.$sp').directive('spVariableLayout', function() {
  ...
  return {
    ...
    controller: function($scope, $rootScope, $timeout) {
      ...
      var _ignoreMandatoryFieldErrors = false;
      var _fieldValidationErrorsHandled = false;
      ...
      function uiMessageHandler(g_form, type, message) {
        switch (type) {
          ...
          case 'errorMessage':
            ...
            _fieldValidationErrorsHandled = true;
            break;
          case 'mandatoryMessage':
            if (_ignoreMandatoryFieldErrors) {
              break;
            }
            spUtil.addErrorMessage(getErrorMessage(message));
            break;
      ...
      $rootScope.$on('sp.form.ui_action.triggered', function() {
        _fieldValidationErrorsHandled = false;
      });
      ...
      $rootScope.$on('sp.form.submit.failed', function(action) {
        if (_fieldValidationErrorsHandled) {
          return;
        }
        _ignoreMandatoryFieldErrors = true;
        g_form.$private.validateForm(action.action_name || action.sys_id);
        _ignoreMandatoryFieldErrors = false;
      });
...
angular.module('sn.$sp').factory('spGtd', function($q, $http, $rootScope, spUtil) {
  "use strict";
  var url = '/api/now/guided_tours/autolaunch/tour';
  try {
    top.NOW = top.NOW || {};
  ...
  function _loadScript(options, tours, state) {
    ...
    if (!top.NOW.guidedToursService) {
      ...
      script.src = '/scripts/app.guided_tours/js_guided_tours_includes.js';
      ...
      _getTourData(options.portal.sys_id, options.page.sys_id).then(
        function(data) {
          ...
          data = data.filter(function(t) {
            let contt = getUrlParameterWithName(t.context, 'id');
