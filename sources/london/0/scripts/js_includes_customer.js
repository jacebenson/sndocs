...
CommonUtils = function() {
  ...
  var join = curry(function(delim, list) {
    if (!list) return '';
    return list.join(delim);
  });
  var map = curry(function(func, list) {
    if (!list) return;
  ...
  var reduce = curry(function(func, list, startArg) {
    if (!list) return;
    if (startArg)
      return list.reduce(func, startArg);
    return list.reduce(func);
  });
  var filter = curry(function(func, list) {
    return list.filter(func);
  });
  var identity = function(x) {
    return x;
  };
  var getRecords = curry(function getRecords(callback, tableName, query, fields) {
    if (!tableName || !query || !fields || !fields.length) return;
    ...
    var ga = new GlideAjax('GlideBatchRecord');
    ga.addParam('sysparm_name', 'processData');
    ga.addParam('sysparm_recordInfos', JSON.stringify([{
      "operation": "read",
      "requiredFields": fields,
      "encodedQuery": query,
      "tableName": tableName
    }]));
    if (callback) return ga.getXML(callback);
    var responseXML = ga.getXMLWait();
    var result = JSON.parse(responseXML.getElementsByTagName('result')[0].attributes.batchResponse.value);
    if (result && result[0] && result[0].data && result[0].data[0])
      return result[0].data;
  });
  var getRecord = curry(function(callback, tableName, sysId, fields) {
    if (!sysId) return;
    if (typeof(sysId) == 'function') sysId = sysId();
    var records = getRecords(callback, tableName, "sys_id=" + sysId, fields);
    if (records && records.length)
      return records[0];
  });
  var getRecordAsync = curry(function(tableName, sysId, fields, callback) {
    if (!sysId) return;
    if (typeof(sysId) == 'function') sysId = sysId();
    var records = getRecords(callback, tableName, "sys_id=" + sysId, fields);
    if (records && records.length)
  ...
  var getRecordWait = getRecord(false);
  var getRecordFieldValueWait = curry(function(tableName, sysId, field) {
    return compose(
      prop(field),
      function(field) {
        return getRecordWait(tableName, sysId, [field]);
      }
    )(field);
  });
  var getFieldValuesWait = getRecordWait(function() {
      return g_form.tableName;
    },
    function() {
      return g_form.getUniqueValue();
    });
  var getFieldValues = getRecordAsync(function() {
      return g_form.tableName;
    },
    function() {
      return g_form.getUniqueValue();
    });
  var getFieldValueWait = function(fieldName) {
    return compose(
      prop(fieldName),
      function(fieldName) {
        return getFieldValuesWait([fieldName]);
      }
    )(fieldName);
  };
  var isFieldModified = function(fieldName) {
    return g_form.tableName + '.' + fieldName in g_form.modifiedFields;
  };
  var unEscapeHTML = function(str) {
    return str.replace(/&amp;(amp;)*/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">");
  };
  var callGlideAjax = curry(function(className, methodName, params, callback) {
    var ga = new GlideAjax(className);
    ga.addParam('sysparm_name', methodName);
    Object.keys(params).forEach(function(name) {
      ga.addParam("sysparm_" + name, params[name]);
    }, this);
    if (callback)
      return ga.getXML(callback);
    ga.getXMLWait();
    return ga;
  });
  var putClientDataWait = curry(function(key, value) {
    return callGlideAjax('CommonAjaxUtils', 'putClientData', {
      key: key,
      value: value
    }, false);
  });
  var getClientDataWait = function(key) {
    return callGlideAjax('CommonAjaxUtils', 'getClientData', {
      key: key
    }, false).getAnswer();
  };
  return {
    ...
    getRecordWait: getRecordWait,
    getFieldValuesWait: getFieldValuesWait,
    getFieldValueWait: getFieldValueWait,
    getRecordFieldValueWait: getRecordFieldValueWait,
    isFieldModified: isFieldModified,
    callGlideAjax: callGlideAjax,
    putClientDataWait: putClientDataWait,
    getClientDataWait: getClientDataWait
  };
}();
  ...
  try {
    ...
    if (isCommunityDomain) {
      if (window.parent.NOW.userDomainDetails && !window.parent.NOW.userDomainDetails.isPureCommunityUser) {
        return;
      } else if (pathName === '/login.do' || pathName === '/login_redirect.do' || pathName == '/navpage.do') {
    ...
    var domain = window.location.href.indexOf("community.servicenow.com") > -1 ? "community" : "hisp";
    if (domain === "community") {
      window.parent.location.replace('/community');
    } else {
