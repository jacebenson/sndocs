...
window.g_tinymce_events = {};

function addTinymceLoadEvent(id, func) {
  if (typeof g_tinymce_events[id] == 'undefined') {
    g_tinymce_events[id] = {
      initialized: false,
      listeners: []
    };
  }
  if (g_tinymce_events[id].initialized === false) {
    g_tinymce_events[id].listeners.push(func);
  } else {
    setTimeout(func, 0);
  }
}

function fireTinymceLoadEvent(id) {
  if (typeof g_tinymce_events[id] == 'undefined') {
    g_tinymce_events[id] = {
      initialized: true,
      listeners: []
  ...
  var evt = g_tinymce_events[id];
  for (var i = 0, l = evt.listeners.length; i < l; i++) {
    var func = evt.listeners[i];
    setTimeout(func, 0);
  }
