...
var GlideForm = Class.create({
    ...
    while (tr) {
      if (tr.nodeName == "TR") {
        if (tr.getAttribute("is_radio_option") == "true") {
          ...
          if (!fieldName.startsWith("IO:") && !fieldName.startsWith("ni.VE"))
  ...
  _setDecorations: function(fieldName, decorations) {
    ...
    if (labelEl) {
      ...
      for (var i = 0; i < maxi; i++) {
        ...
        decs += '<span class="field_decoration ' + dec.icon + '" title="' + dec.text + '"></span>';
      }
      decorEl.html(decs);
      return true;
    }
  ...
  showFieldMsg: function(input, message, type, scrollForm) {
    ...
    if (!fieldRow) {
      ...
      while (parent != null) {
        if (parent.nodeName.toUpperCase() == 'TD') {
          ...
          prevSiblings = $j(parent).prevAll().length;
        }
      ...
      if (fieldRow) {
        ...
        if (prevSiblings > 0) {
          var prevSiblingsCell = doc.createElement('TD');
          prevSiblingsCell.colSpan = prevSiblings;
          variableMsgRow.appendChild(prevSiblingsCell);
        }
    ...
    if (isVariable) {
      ...
      if (!positionedCursor)
        this._scrollToElementTR(variableMsgRow, fieldMsgRow, scrollForm);
    } else {
