...
angular.module('sn.$sp').factory('spLabelHelper', function($q, $http, i18n, spUtil, glideFormFieldFactory) {
  ...
  function preparePriceMap(field) {
    ...
    for (var i = 0; i < field.choices.length; i++) {
      ...
      if (typeof field.choices[i].price_dc !== 'undefined')
        key1 += '_' + field.choices[i].price_dc;
      if (typeof field.choices[i].rprice_dc !== 'undefined')
...
angular.module('sn.$sp').directive('spDatePicker', function(spConf, $rootScope, $document, $window, spAriaUtil, i18n, spDatePickerUtil, select2EventBroker) {
  ...
  function detachKeyboardEvent(picker) {
    ...
    picker.widget.off('click', '.datepicker .day div');
  }
  ...
  return {
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      var format, initDateTimePicker = true;
...
angular.module('sn.$sp').directive('spReferenceElement', function($http, spUtil, filterExpressionParser, escapeHtml, i18n, spIs, spAriaUtil, select2EventBroker) {
  "use strict";
...
angular.module('sn.$sp').directive('spVariableLayout', function() {
  ...
  spModal, glideListFactory, spModelUtil, spLabelHelper, $sce, spAtf, spSCFieldPropertyDecorator, spSCConf, $rootScope, uiScriptFactory) {
  'use strict';
  return {
    restrict: 'E',
...
angular.module('sn.$sp')
  ...
  .directive('spRating', function($timeout) {
    return {
      restrict: 'A',
      link: function(scope, elem, attrs) {
        $timeout(function() {
...
angular.module('sn.$sp').directive('spAnnouncements', function() {
  return {
    ...
    controller: function($scope, spAnnouncement, spAriaUtil, i18n) {
      ...
      c.getDisplayStyleValue = function(announcement) {
        return (announcement.displayStyle || {}).alignment === 'CENTER' ? 'flex' : '';
      };
...
angular.module('sn.$sp').factory('spGtd', function($q, $http, $rootScope, spUtil) {
  "use strict";
      ...
      _getTourData(options.portal.sys_id, options.page.sys_id).then(
        function(data) {
          ...
          data = data.filter(function(t) {
            ...
            return (filterByPageId ? (contt.value === pageid.value) : true);
          });
