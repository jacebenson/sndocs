...
function addAttachmentNameToForm(sysid, name, hoverText, image, showRename, showView, showPopup) {
  ...
  if (attachList.hasClassName("with-ellipsis"))
  ...
  if (showView == "true") {
    ...
    var downloadAttachmentElement = gel("ni.download_attachment");
    var downloadAttachment = downloadAttachmentElement ? downloadAttachmentElement.value : "false";
    if (downloadAttachment === "true")
      getMessage("Download {0}", setAriaLabel);
    else
      getMessage("View {0}", setAriaLabel);
    var newText = document.createTextNode(downloadAttachment === "true" ? getMessage("[download]") : getMessage("[view]"));
...
function addAttachmentNameToDialog(id, fileName, canDelete, createdBy, createdOn, contentType, iconPath) {
  ...
  if (showView == "true") {
    ...
    var downloadAttachmentElement = gel("ni.download_attachment");
    var downloadAttachmentAction = downloadAttachmentElement ? downloadAttachmentElement.value : "false";
    if (downloadAttachmentAction === "true")
      getMessage("Download {0}", setAriaLabel);
    else
      getMessage("View {0}", setAriaLabel);
    var newText = document.createTextNode(downloadAttachmentAction === "true" ? getMessage("[download]") : getMessage("[view]"));
    view.appendChild(newText);
