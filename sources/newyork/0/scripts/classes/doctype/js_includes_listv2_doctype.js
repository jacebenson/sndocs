...
var GlideList2 = Class.create(GwtObservable, {
  ...
  getOriginalView: function() {
    var url = new GlideURL(window.location.href);
    return url.getParam('sysparm_original_view') || '';
  },
  ...
  _action: function(actionId, actionName, ids, isClientSide) {
    ...
    if (this.getView() === 'sys_ref_list' && actionName === 'sysverb_new')
      this.addToForm('sysparm_view', this.getOriginalView());
    else
...
var GlideWidgetVCR = Class.create(GlideListWidget, {
  ...
  _setRowNumbers: function(list) {
    ...
    if (lastRow)
      lastRow.innerHTML = list.lastRow.toLocaleString();
    if (totalRows)
...
CustomEvent.observe('list2_init', function(list2) {
  ...
  function cloneHeader() {
    ...
    var $pageHeader = $j('.list_nav_top.list_nav[role="toolbar"]').first();
    ...
    $cloneTable.find('[tabindex],button,a').each(function(i, elem) {
      $j(elem).attr('tabindex', -1).attr('aria-hidden', true);
    });
