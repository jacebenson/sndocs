...
function onSelChange(elementName) {
  ...
  for (var i = 0; i < eDeps.length; i++) {
    ...
    var retFunc = selResponseHelper(includesSysSelect);
...
function selResponse(request, containsSysSelect) {
  ...
  var noneText = getMessage('-- None --');
  if (processorName === "PickList")
    appendSelectOption(select, "", document.createTextNode((defaultOption ? defaultOption : noneText)));
...
function selResponseHelper(hasSysSelect) {
  return function(request) {
    selResponse(request, hasSysSelect);
  }
}
