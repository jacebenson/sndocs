...
angular.module('sn.common.avatar').directive('snAvatar', function($http, $compile, getTemplateUrl, snCustomEvent) {
  'use strict';
  return {
    restrict: 'E',
          ...
          if (scope.users && scope.users.length === 1 && scope.users[0] && (scope.users[0].userID || scope.users[0].sys_id)) {
            ...
            if ((gUser && scope.users[0].userID && scope.users[0].userID !== gUser.userID) ||
              ...
              scope.contextOptions.push(["Open a new chat", function() {
                snCustomEvent.fireTop('chat:open_conversation', scope.users[0]);
              }]);
            }
          }
        } else {
    ...
    controller: function($scope, liveProfileID, $timeout) {
      $scope.displayMemberCount = $scope.displayMemberCount || false;
      $scope.liveProfileID = liveProfileID;
  ...
  this.$get = function linkContentTypesFactory() {
    return {
      forType: function(link) {
        ...
        if (!link.isActive && !link.isPending)
...
angular.module("sn.common.presence").factory('snRecordPresence', function($rootScope, $location, amb, $timeout, $window, PRESENCE_DISABLED) {
  "use strict";
  var statChannel;
...
angular.module("sn.common.stream").directive("snStream", function(getTemplateUrl, $http) {
  "use strict";
  return {
    restrict: "E",
    replace: true,
    ...
    controller: function($scope, $attrs, nowStream, snRecordPresence, snCustomEvent, userPreferences, $window, $q, $timeout, snMention) {
      ...
      function processEntries(entries) {
        ...
        if ($scope.loaded) {
          ...
          triggerResize();
        } else if ($scope.entries.length > chunksize)
          chunkIt(chunksize);
      }

      function chunkIt(chunksize) {
        var original = $scope.entries;
        $scope.entries = [];
        var index = 0;
        z();

        function z() {
          var max = chunksize + index;
          if (original.length < max)
            max = original.length;
          for (var i = index; i < max; i++)
            $scope.entries.push(original[i]);
          index = max;
