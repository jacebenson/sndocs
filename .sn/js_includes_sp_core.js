...
angular.module('sn.$sp').factory('spLabelHelper', function($q, $http, i18n, spUtil) {
    'use strict'

    function getReferenceLabelContents(field) {
        var label = "";
        if (field.isMandatory() && !field.mandatory_filled()) {
...
angular.module('sn.$sp').directive('spChoiceList', function($timeout, spUtil, $http) {
    return {
        template: '<select name="{{::field.name}}" id="sp_formfield_{{::field.name}}" ng-model="fieldValue" ng-model-options="{getterSetter: true}" sn-select-width="auto" ng-disabled="field.isReadonly()" ng-options="c.value as (c.label.concat(c.priceLabel)) for c in field.choices track by getVal(c.value)"></select>',
        restrict: 'E',
        replace: true,
        require: 'ngModel',
        scope: {
...
angular.module('sn.$sp').directive('spVariableLayout', function() {
    ...
    spModal, glideListFactory, spModelUtil) {
    'use strict';
    return {
        restrict: 'E',
        ...
        controller: function($scope) {
            ...
            $scope.execItemScripts = _.once(function() {
...
angular.module('sn.$sp')
    ...
    .directive('a', function(spAriaUtil) {
        return {
            restrict: 'E',
            link: spAriaUtil.link('link', false, 0)
        }
    })
    ...
    .directive('button', function(spAriaUtil) {
        return {
            restrict: 'E',
            link: spAriaUtil.link('button', false, 0)
        }
    });
angular.module('sn.$sp').directive('header', function(spAriaUtil) {
        return {
            restrict: 'E',
            link: spAriaUtil.link('banner')
        }
    })
    .directive('footer', function(spAriaUtil) {
        return {
            restrict: 'E',
            link: spAriaUtil.link('contentinfo')
        }
    })
    .directive('nav', function(spAriaUtil) {
        return {
            restrict: 'E',
            link: spAriaUtil.link('navigation')
        }
    })
