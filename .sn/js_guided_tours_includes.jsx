...
var evtObj = {event: name, tour: service.currentSysId, timestamp: Date.now(), details: ''};
switch(name) {
...
if (tour.steps[i].window == tour.steps[i+1].window && tour.steps[i].implicit) {
} else if (tour.steps[i].window == tour.steps[i+1].window) {
tour.steps[i].onNext.push(["switchFrame", tour.sys_id, i+1]);
}
}
}
tour.steps[i].onShow = [["scrollToView"],["setFocus"]];
...
if (step === 0)
eventEmitter.emit(eventStreamName, eventCreationHelper.createEventData(eventNames.started, this, {tour: tour, step: step}));
eventEmitter.emit(eventStreamName, eventCreationHelper.createEventData(eventNames.stepStarted, this, {tour: tour, step: step}));
...
_checkForAutoLaunchTour: function(){
var self = this;
var currentAutoLaunchTour = GuidedToursService.currentAutoLaunchTour;
...
dismissAutoLaunchModal = new win.GlideModal('');
dismissAutoLaunchModal.setTitle('Stop Guided Tour');
dismissAutoLaunchModal.renderWithContent(DismissAutoLaunch.template());
...
eventEmitter.emit(eventStreamName, eventCreationHelper.createEventData(eventNames.failed, this, {message: msg}));
this.hasFailed = true;
...
top.g_GlideUI.addOutputMessage({'msg': msg, 'type': msgType});
} else {
